const App = () => {
  ...
  const scheduleStepWork = useCallback((callback, delay) => {
  ...
  const handleSliderReleaseComplete = useCallback((finalK) => {
    ...
    if (step === 1 && Math.abs(finalK - cfg.max) < 0.02) {
      scheduleStepWork(() => setCurrentStep(2), 300);
    } else if (step === 2 && Math.abs(finalK - cfg.center) < 0.02) {
      scheduleStepWork(() => setCurrentStep(3), 300);
      ...
      // slider stays live while the MCQ is up - learner can keep exploring k
    } else if (
      step === 6 &&
  ...
  const resetEverything = useCallback(() => {
    ...
    setNudgePositions([]);
  }, [clearScheduledTimeouts, cancelAnimation, cancelPulse, resetMcq]);

  const resetForFreshStep = useCallback(
    (targetStep) => {
      clearScheduledTimeouts();
      cancelAnimation();
      cancelPulse();
      ...
      if (targetStep <= 1) {
        setSliderK(INTRO_SLIDER.default);
        setVisualK(INTRO_SLIDER.default);
        visualKRef.current = INTRO_SLIDER.default;
        setShowGhost(false);
        setStep1Enlarged(false);
      } else if (targetStep === 2) {
        setSliderK(INTRO_SLIDER.max);
        setVisualK(INTRO_SLIDER.max);
        visualKRef.current = INTRO_SLIDER.max;
        setShowGhost(true);
        setStep1Enlarged(true);
      } else if (targetStep === 3) {
        setSliderK(INTRO_SLIDER.center);
        setVisualK(INTRO_SLIDER.center);
        visualKRef.current = INTRO_SLIDER.center;
        setShowGhost(false);
        setStep1Enlarged(true);
      } else if (targetStep === 4) {
        setSliderK(INTRO_SLIDER.min);
        setVisualK(INTRO_SLIDER.min);
        visualKRef.current = INTRO_SLIDER.min;
        setShowGhost(true);
        setStep1Enlarged(true);
  ...
  const handlePrev = () => {
    ...
    if (typeof playSound === "function") playSound("click");
    if (currentStep > 1) resetForFreshStep(currentStep - 1);
  };
  ...
  const sliderMode = useMemo(() => {
    ...
    if (currentStep === 5 || currentStep === 6) return "positive";
    if (currentStep === 7) return "negative";
    return "both";
  }, [currentStep]);
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(MainCanvas, {
        ...
        sliderLocked: sliderLocked || isAnimating,
