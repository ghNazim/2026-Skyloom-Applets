const RotationSvg = ({
  ...
  const shapeType =
    step === 3 ? "rhombus" : step === 4 ? "trapezium" : "triangle";
  const basePoints =
    shapeType === "rhombus"
      ? getBaseRhombusPoints()
      : shapeType === "trapezium"
        ? getBaseTrapeziumPoints()
        : getBaseTrianglePoints();

  const orangeAngle = 0;
  const blueAngle =
    (step === 4 ? -rotationAngle : rotationAngle) + 180;
  ...
  const rotationArc =
    showRotationArrow && arrowDrawProgress > 0
      ? getRotationArcPath(rotationArrowDirection, arrowDrawProgress)
      : "";
