...
function wrapMathVars(text) {
  if (typeof text !== "string") return text;
  return text
    .replace(/\bu\b/g, "<span class='math-var'>u</span>")
    .replace(/\bv\b/g, "<span class='math-var'>v</span>");
}

function decorateMathVars(node) {
  if (typeof node === "string") return wrapMathVars(node);
  if (Array.isArray(node)) return node.map(decorateMathVars);
  if (!node || typeof node !== "object") return node;

  Object.keys(node).forEach((key) => {
    if (key === "labels") return;
    node[key] = decorateMathVars(node[key]);
  });
...
Object.keys(DATA).forEach((lang) => {
  decorateMathVars(DATA[lang].app);
});
