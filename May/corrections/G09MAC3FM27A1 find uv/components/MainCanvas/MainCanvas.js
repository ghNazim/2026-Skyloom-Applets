...
function getPqrsTransformForStep(step) {
  return {
    centroid: { ...PQRS_GEOM.alignedCentroid },
    rotation: step >= 5 ? 0 : PQRS_GEOM.initialRotation,
  };
}
...
const MainCanvas = (props) => {
  ...
  const resetPqrsToInitial = useCallback(() => {
    setPqrsTransform({
      centroid: { ...PQRS_GEOM.alignedCentroid },
  ...
  const resetPqrsToAligned = useCallback(() => {
    setPqrsTransform({
      centroid: { ...PQRS_GEOM.alignedCentroid },
      rotation: 0,
    });
  ...
  useLayoutEffect(() => {
    ...
    if (step === 5) {
      ...
      setActionText(APP_DATA.steps[5].actionText);
      setLabeledVertices({});
      setNextPqrsLabel(0);
      setOrangeAngles({});
      setQAngleFound(false);
    ...
    if (step === 6) {
      ...
      setRotationDone(true);
      setActionText(null);
      setLabeledVertices({ P: "P", Q: "Q", R: "R", S: "S" });
      setNextPqrsLabel(4);
      setOrangeAngles({});
      setQAngleFound(false);
      setSAangleFound(false);
  ...
  const runRotationAnimation = () => {
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        onSetNextEnabled(true);
        setActionText(APP_DATA.steps[4].actionText);
        if (onAnimationStateChange) {
          onAnimationStateChange(false);
        }
      },
  ...
  const handleNumpadSubmit = () => {
    ...
    if (val === 70) {
      ...
      setVFound(true);
      setOrangeAngles((prev) => ({ ...prev, P: true }));
      onSetNextEnabled(true);
      onUpdateTexts(
  ...
  const highlightVAtStep10 = step === 10;
  const showAbcLabels = step >= 5;
  const abcDimmed = step === 9 || step === 10;
  const summarizeOrange = step === 11;
    ...
    return React.createElement(
      ...
      hasMathVar
        ? [
            React.createElement(
              "tspan",
              { key: "var", className: "math-var-svg" },
              text.charAt(0),
            ),
            text.slice(1)
              ? React.createElement("tspan", { key: "rest" }, text.slice(1))
              : null,
          ]
        : text,
    );
  ...
  const renderOrangeVertexHotspot = (vertexKey, pts) => {
  ...
  const renderPqrsAngles = () => {
    ...
    const renderP = () => {
      ...
      elems.push(
        renderAngleArc(p.P, p.S, p.Q, {
          fill,
          stroke: COLOR_WHITE,
          strokeWidth: summarizeOrange || isOrange ? 2.5 : 2,
          id: "pqrs-angle-P",
        }),
      );
    ...
    const renderS = () => {
      const shouldHighlightOrange = highlightUV || summarizeOrange;
      ...
      elems.push(
        renderAngleArc(p.S, p.R, p.P, {
          fill,
          stroke: COLOR_WHITE,
          strokeWidth: summarizeOrange || isOrange || isGlow ? 2.5 : 2,
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          showPqrsVertexHotspots &&
            VERTEX_ORDER_PQRS.map((k) =>
              renderOrangeVertexHotspot(k, pqrsPoints),
            ),
        ),
      ),
    ),
