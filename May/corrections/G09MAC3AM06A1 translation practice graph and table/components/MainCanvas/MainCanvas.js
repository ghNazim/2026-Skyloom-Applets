...
const STEP2_TO_TABLE_TRANSFERS = [
  { labelKey: "P", cellKey: "pre-p", text: () => APP_DATA.graph.labelP },
  { labelKey: "Q", cellKey: "pre-q", text: () => APP_DATA.graph.labelQ },
  { labelKey: "R", cellKey: "pre-r", text: () => APP_DATA.graph.labelRUnknown },
  { labelKey: "PPrime", cellKey: "img-p", text: () => APP_DATA.graph.labelPPrime },
  {
    labelKey: "QPrime",
    cellKey: "img-q",
    text: () => APP_DATA.graph.labelQPrimeUnknown,
  },
  { labelKey: "RPrime", cellKey: "img-r", text: () => APP_DATA.graph.labelRPrime },
];
...
const MainCanvas = ({
  ...
  const [step10OutlinedCells, setStep10OutlinedCells] = useState({
    r: false,
    qPrime: false,
  });
  ...
  const step2To3TransferRef = useRef({
    token: 0,
    pending: false,
    clones: [],
  });
  const [step3VisibleCells, setStep3VisibleCells] = useState(null);
  ...
  const makeStep3Visibility = useCallback((visible) => {
    return STEP2_TO_TABLE_TRANSFERS.reduce((acc, item) => {
      acc[item.cellKey] = visible;
      return acc;
    }, {});
  }, []);
  ...
  useEffect(() => {
    if (!step2To3TransferId || step !== 2) return;
    if (step2To3TransferRef.current.token === step2To3TransferId) return;

    const clones = STEP2_TO_TABLE_TRANSFERS.map((item) => {
      const sourceEl = labelRefs.current[item.labelKey];
      if (!sourceEl) return null;
      const rect = sourceEl.getBoundingClientRect();
      const computed = window.getComputedStyle(sourceEl);
      const id = "step2-table-" + item.cellKey + "-" + step2To3TransferId;
      return {
        id: id,
        mode: "text",
        text: item.text(),
        targetKey: item.cellKey,
        startX: rect.left + rect.width / 2,
        startY: rect.top + rect.height / 2,
        dx: 0,
        dy: 0,
        color: computed.fill || computed.color,
        fontSize: "1.9vw",
        fontWeight: "700",
        animating: false,
      };
    ...
    step2To3TransferRef.current = {
      token: step2To3TransferId,
      pending: clones.length > 0,
      clones: clones,
    };
    setStep3VisibleCells(makeStep3Visibility(false));
    setFlyClones((prev) => prev.concat(clones));
    if (clones.length) {
      setPointStates((prev) => {
        const next = { ...prev };
        STEP2_TO_TABLE_TRANSFERS.forEach((item) => {
          if (!next[item.labelKey]) return;
          next[item.labelKey] = {
            ...next[item.labelKey],
            labelVisible: false,
          };
    ...
    setTimeout(() => {
      if (typeof onStep2To3Ready === "function") onStep2To3Ready();
      if (!clones.length && typeof onStep2To3Complete === "function") {
        onStep2To3Complete();
      }
    }, 150);
  }, [
  ...
  useEffect(() => {
    const transfer = step2To3TransferRef.current;
    if (step !== 3 || !transfer.pending || !transfer.clones.length) {
      if (step === 3 && !transfer.pending) {
        setStep3VisibleCells(null);
      }
      return;
    }
    ...
    const runTransfer = async () => {
      ...
      transfer.clones.forEach((clone) => {
        const targetEl = cellRefs.current[clone.targetKey];
        if (!targetEl) {
          return;
        }
        const rect = targetEl.getBoundingClientRect();
        const dx = rect.left + rect.width / 2 - clone.startX;
        const dy = rect.top + rect.height / 2 - clone.startY;
        setFlyClones((prev) =>
          prev.map((item) =>
            item.id === clone.id
              ? { ...item, dx: dx, dy: dy, animating: true }
              : item,
          ),
        );
      });
      ...
      setStep3VisibleCells(makeStep3Visibility(true));
      setFlyClones((prev) =>
        prev.filter((item) => !transfer.clones.some((clone) => clone.id === item.id)),
      );
      step2To3TransferRef.current = {
      ...
      if (typeof onStep2To3Complete === "function") onStep2To3Complete();
    };
  ...
  }, [step, makeStep3Visibility, onStep2To3Complete]);
  ...
  useEffect(() => {
    ...
    if (step === 10) {
      ...
      if (step10Phase === "initial") {
        ...
        setStep10OutlinedCells({ r: false, qPrime: false });
      }
    }
    if (step < 7) {
    ...
    if (showUnknown) {
      ...
      const qOpacity =
        pointStates.QPrime && pointStates.QPrime.opacity != null
          ...
          : 1;
      const rLabelOpacity =
        !pointStates.R || pointStates.R.labelVisible !== false ? 1 : 0;
      const qLabelOpacity =
        !pointStates.QPrime || pointStates.QPrime.labelVisible !== false ? 1 : 0;
      pts.push({
        id: "R",
        x: unknownR.x,
        y: unknownR.y,
  ...
  const leftTableProps = {
    ...
    hiddenCoordCells:
      step === 3 && step3VisibleCells
        ? STEP2_TO_TABLE_TRANSFERS.reduce((acc, item) => {
            acc[item.cellKey] = !step3VisibleCells[item.cellKey];
            return acc;
          }, {})
        : null,
  };
  ...
  const rightTableProps = {
    ...
    qPrimeKnown: true,
    rKnown: true,
    outlinedCells: step10OutlinedCells,
  };
