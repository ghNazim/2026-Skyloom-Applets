...
const APP_IMAGE_GRAPH_COLOR = "#46c5ce";
const APP_OBJECT_GRAPH_COLOR = "#fb9b5b";
...
const App = () => {
  ...
  const [step6FigureColor, setStep6FigureColor] = useState(APP_IMAGE_GRAPH_COLOR);
  ...
  const resetStep6State = useCallback(() => {
    ...
    setStep6FigureColor(APP_IMAGE_GRAPH_COLOR);
  ...
  const handleFigureDrop = useCallback(
    (dx, dy) => {
      ...
      if (isCorrect) {
        if (typeof playSound === "function") playSound("correct");
        setStep6FigureColor(APP_OBJECT_GRAPH_COLOR);
