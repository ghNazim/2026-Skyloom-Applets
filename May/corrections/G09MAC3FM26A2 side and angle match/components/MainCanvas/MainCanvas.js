...
const MainCanvas = (props) => {
  ...
  const [showDefGhost, setShowDefGhost] = useState(false);
  ...
  const ghostPoints = useMemo(() => {
    const side = sideTransformRef.current;
    return getDefPointsFromTransform(side.centroid, side.rotation);
  }, []);
  ...
  const actionRowContent = () => {
    if (showFeedback) {
      return React.createElement("div", {
        className:
          "overlap-feedback-box overlap-feedback-box--action" +
