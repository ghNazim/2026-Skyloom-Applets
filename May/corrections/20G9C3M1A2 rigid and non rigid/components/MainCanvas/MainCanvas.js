const MainCanvas = (props) => {
  ...
  const getLeftCardNudgePoint = (ox, oy, leg) => {
    const centroid = getTriCentroid(ox, oy, leg);
    return {
      x: centroid.x + leg * 0.42,
      y: centroid.y - leg * 0.04,
    };
  };
  ...
  const getActiveTriangleNudgePoint = () => {
    ...
    if (step === 2 && bigTrianglesVisible && !translationDone) {
      const origin = getCardBlueOrigin("translation");
      return getLeftCardNudgePoint(origin.x, origin.y, TRI_LEG);
    }
    if (step === 3 && bigTrianglesVisible && !reflectionDone) {
      const origin = getCardBlueOrigin("reflection");
      return getTriCentroid(origin.x, origin.y, TRI_LEG);
    }
    if (step === 4 && bigTrianglesVisible && !rotationDone) {
      const origin = getCardBlueOrigin("bottomRight");
      return getTriCentroid(origin.x, origin.y, TRI_LEG);
    }
    if (step === 5 && bigTrianglesVisible && !dilationDone) {
      const origin = getCardBlueOrigin("bottomLeft");
      return getLeftCardNudgePoint(origin.x, origin.y, TRI_LEG);
    }
    return null;
  };
  ...
  function getCardBlueOrigin(key) {
    const cx = CARD_CENTERS[key].x;
    const cy = CARD_CENTERS[key].y;

    if (key === "translation") {
      return getTriOriginBottom(cx, cy, BIG_CARD_W, BIG_CARD_H, TRI_LEG, "left");
    }
    if (key === "reflection") {
      return getReflBlueOrigin(cx, cy, BIG_CARD_W, BIG_CARD_H, TRI_LEG);
    }
    if (key === "bottomRight") {
      return getRotBlueOrigin(cx, cy, BIG_CARD_W, BIG_CARD_H, TRI_LEG);
    }
    if (key === "bottomLeft") {
      return getDilBlueOrigin(cx, cy, BIG_CARD_W, BIG_CARD_H, TRI_LEG);
    }
    return getTriOriginCentered(cx, cy, TRI_LEG);
  }
  ...
  const renderCloneTriangles = () => {
    ...
    return React.createElement(
      ...
      BIG_CARD_KEYS.map((key) => {
        const target = getCardBlueOrigin(key);
