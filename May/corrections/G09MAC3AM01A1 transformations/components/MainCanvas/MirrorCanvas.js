const MirrorCanvas = function (props) {
  ...
  var onTextLayoverActive = props.onTextLayoverActive;
  ...
  var reflectionInViewState = useState(false);
  var reflectionInView = reflectionInViewState[0];
  var setReflectionInView = reflectionInViewState[1];
  ...
  var summaryTextCompleteState = useState(false);
  var summaryTextComplete = summaryTextCompleteState[0];
  var setSummaryTextComplete = summaryTextCompleteState[1];
  ...
  var handlePointerMove = useCallback(
    function (e) {
      ...
      var reflected = reflectPointsAcrossX(leftPoints, nextX);
      var inView = isReflectionInViewbox(reflected);
      setReflectionInView(inView);
      if (inView && onUpdateQuestion) {
        onUpdateQuestion("Move the mirror so the reflected image reaches the target");
      }
      trySnapMirror(nextX);
    },
    [
  ...
  var handleBlueBoxClick = useCallback(
    function () {
      ...
      setSummaryVisible(false);
      if (onNudgeDismiss) onNudgeDismiss();
      if (onTextLayoverActive) onTextLayoverActive(true);
    ...
    [onNudgeDismiss, onUpdateQuestion, MIRROR.questionTranslation, onTextLayoverActive]
  );
  ...
  var handleTextLayoverClose = useCallback(
    function () {
      setTextLayoverVisible(false);
      setTimeout(function () {
        if (onTextLayoverActive) onTextLayoverActive(false);
    ...
    [startDefinitionTyping, onMirrorFlowComplete, onTextLayoverActive]
  );
  ...
  useEffect(
    function () {
      if (showBlueBox && summaryTextComplete && onNudgeTarget) {
        requestAnimationFrame(function () {
          requestAnimationFrame(function () {
            onNudgeTarget("blue");
          });
        });
      } else if (showRevealBtn && summaryTextComplete && onNudgeTarget) {
        onNudgeTarget("mirrorReveal");
      }
    },
    [showRevealBtn, showBlueBox, summaryTextComplete, onNudgeTarget]
  );
  ...
  var renderSummaryContent = function () {
    ...
    return React.createElement(AnimatedSummaryElement, {
      summary: currentSummary,
      className: "mirror-summary-box__typing",
      onComplete: function () {
        setSummaryTextComplete(true);
      },
    });
  };
    ...
    React.createElement(
      ...
      showStaticYellow &&
        React.createElement("path", {
          ...
          opacity: reflectionInView && !staticYellowSnapped ? 0.3 : 1,
          stroke: IMAGE_COLOR,
          strokeWidth: 5,
