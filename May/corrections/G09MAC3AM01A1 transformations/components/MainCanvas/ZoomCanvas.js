const ZoomCanvas = function (props) {
  ...
  useEffect(
    function () {
      ...
      } else if (showBlueBox && summaryTextComplete && onNudgeTarget) {
        requestAnimationFrame(function () {
          requestAnimationFrame(function () {
            onNudgeTarget("blue");
          });
    ...
    [zoomPhase, showBlueBox, showRevealBtn, summaryTextComplete, onNudgeTarget]
  );
  ...
  var handleBlueBoxClick = useCallback(
    ...
    [onNudgeDismiss, onUpdateQuestion, ZOOM.questionTranslation, onTextLayoverActive]
  );
  ...
  var handleTextLayoverClose = useCallback(
    ...
    [startDefinitionTyping, onZoomFlowComplete, onTextLayoverActive]
  );
  ...
  var renderRevealOverlay = function () {
    ...
    var yellowPts = TRACK_KEYS.map(function (key) {
      ...
      return renderMarkerPoint(yellow.x, yellow.y, IMAGE_COLOR, "yellow-" + key);
    });
