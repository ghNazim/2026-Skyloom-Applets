...
const MainCanvas = (props) => {
  ...
  const [revealTypedText, setRevealTypedText] = useState("");
  ...
  const startRevealTyping = useCallback(
    function () {
      var full = STEP3.revealParts.join("");
      var index = 0;
      setRevealTypedText("");

      function typeNext() {
        if (index >= full.length) {
          setRevealTypedText(full);
          return;
        }
        index = getNextTypedHtmlIndex(full, index);
        setRevealTypedText(full.slice(0, index));
  ...
  const handleRevealClick = useCallback(
    ...
    [startRevealPointSequence, startRevealTyping, onNudgeDismiss]
  );
  ...
  const handleBlueBoxClick = useCallback(
    ...
    [onUpdateQuestion, STEP3.questionTranslation, onNudgeDismiss, onTextLayoverActive]
  );
  ...
  const handleTextLayoverClose = useCallback(
    ...
    [startDefinitionTyping, onMoveFlowComplete, onTextLayoverActive]
  );
  ...
  useEffect(
    function () {
      ...
      if (showBlueBox) {
        ...
        onNudgeTarget("moveReveal");
      }
    },
    [showBlueBox, greyClickable, showRevealBtn, onNudgeTarget]
  );
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement("stop", {
            offset: "0%",
            stopColor: OBJECT_COLOR,
          }),
          React.createElement("stop", {
            offset: "100%",
            stopColor: IMAGE_COLOR,
          })
        )
      ),
  ...
  var renderMovingVertexPoint = function (key) {
    ...
    var innerColor = progress >= 0.98 ? IMAGE_COLOR : OBJECT_COLOR;
  ...
  var revealBoxContent = showRevealBox
    ? definitionTyped
      ...
      : React.createElement("div", {
          className: "reveal-box__typing",
          dangerouslySetInnerHTML: { __html: revealTypedText },
        })
    : null;
    ...
    React.createElement(
      ...
      showPaths &&
        renderPath(rightPoints, "path-right" + pathDimSuffix, {
          fill: rightPathFilled ? IMAGE_COLOR_LIGHT : "rgba(0,0,0,0.15)",
          stroke: IMAGE_COLOR,
          strokeWidth: 3,
