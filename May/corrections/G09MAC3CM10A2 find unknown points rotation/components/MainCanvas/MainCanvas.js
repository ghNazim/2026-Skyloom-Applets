...
const MainCanvas = ({
  ...
  const step4TransferRef = useRef(false);
  ...
  const [step4LeftPrepInvisible, setStep4LeftPrepInvisible] = useState(false);
  const [step4RightSourceHidden, setStep4RightSourceHidden] = useState(false);
  ...
  const getFlyText = useCallback((el, fallbackText) => {
    if (fallbackText != null) return String(fallbackText).trim();
    if (!el) return "";
        ...
        prev.concat([
          {
            id: id,
            mode: "text",
            text: getFlyText(sourceEl, options.text),
            color: color,
  ...
  const animateTableFly = useCallback((snapshot) => {
    return new Promise((resolve) => {
      if (!snapshot) {
        resolve();
        return;
      }
      const id = snapshot.id || "table-" + Date.now();
        ...
        prev.concat([
          {
            ...
            wrapClassName: snapshot.wrapClassName,
            dx: snapshot.dx,
            dy: snapshot.dy,
            scaleX: snapshot.scaleX,
            scaleY: snapshot.scaleY,
  ...
  const captureTableFlySnapshot = useCallback((sourceEl, targetEl, id) => {
    if (!sourceEl || !targetEl) return null;

    const src = sourceEl.getBoundingClientRect();
    const tgt = targetEl.getBoundingClientRect();

    if (src.width < 2 || src.height < 2 || tgt.width < 2 || tgt.height < 2) {
      return null;
    }

    return {
      id: id || "fly-table-" + Date.now(),
      startX: src.left,
      startY: src.top,
      width: src.width,
      height: src.height,
      innerHTML: sourceEl.innerHTML,
      wrapClassName: sourceEl.className || "piit-wrap is-visible",
      dx: tgt.left - src.left,
      dy: tgt.top - src.top,
      scaleX: 1,
      scaleY: 1,
    };
  ...
  useEffect(() => {
    ...
    if (step === 3) {
      step4TransferRef.current = false;
      setStep4LeftPrepInvisible(false);
      setStep4RightSourceHidden(false);
      setLeftVisible(true);
      setTableVisible(true);
    ...
    if (step === 4) {
      ...
      setStep4LeftPrepInvisible(step4Phase === "transition");
      setRightVisible(true);
    }
    if (step < 5) {
  ...
  useLayoutEffect(() => {
    if (step !== 4 || step4Phase !== "transition" || step4TransferRef.current) {
      return;
    }
    step4TransferRef.current = true;

    let cancelled = false;

    const runStep4Transfer = async () => {
      if (typeof onNavAnimating === "function") onNavAnimating(true);
      setRightVisible(true);
      setStep4RightSourceHidden(false);
      setStep4LeftPrepInvisible(true);
      ...
      const sourceWrap = await waitForElement(
        () => cellRefs.current["right-table-wrap"],
        50,
      );
      const targetWrap = await waitForElement(
        () => cellRefs.current["left-table-wrap"],
        50,
      );
      if (cancelled) return;

      const snapshot = captureTableFlySnapshot(
        sourceWrap,
        targetWrap,
        "step4-table-transfer",
      );

      if (!snapshot) {
        setStep4LeftPrepInvisible(false);
        if (!cancelled && typeof onStep4PhaseChange === "function") {
          onStep4PhaseChange("mcq");
        }
        if (typeof onNavAnimating === "function") onNavAnimating(false);
        return;
      }

      setStep4RightSourceHidden(true);
      await delay(30);
      if (cancelled) return;

      await animateTableFly(snapshot);
      if (cancelled) return;
  ...
  const leftTableProps = {
    ...
    prepInvisible: step === 4 && step4LeftPrepInvisible,
  ...
  const rightTableProps = {
    ...sharedTableProps,
    refPrefix: step === 4 ? "right-" : "",
    wrapRefKey: "right-table-wrap",
    twoColumnOnly: true,
    visible: tableVisible && showRightTable && !step4RightSourceHidden,
    contentVisible: true,
    showRotationColumn: false,
  };
