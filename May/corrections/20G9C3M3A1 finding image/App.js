...
const App = () => {
  ...
  const [step10FigureColor, setStep10FigureColor] = useState(APP_COLORS.object);
  ...
  const resetStep10State = useCallback(() => {
    ...
    setStep10FigureColor(APP_COLORS.object);
  ...
  const handleFigureDrop = useCallback(
    (dx, dy) => {
      ...
      if (isCorrect) {
        if (typeof playSound === "function") playSound("correct");
        setStep10FigureColor(APP_COLORS.image);
