...
function describeArc(cx, cy, r, startRad, rotationDeg) {
  ...
  if (Math.abs(Math.abs(rotationDeg) - 360) < 0.5) {
    const midRad = startRad + rotationRad / 2;
    const midX = cx + r * Math.cos(midRad);
    const midY = cy + r * Math.sin(midRad);
    return [
      `M ${startX} ${startY}`,
      `A ${r} ${r} 0 1 ${sweep} ${midX} ${midY}`,
      `A ${r} ${r} 0 1 ${sweep} ${startX} ${startY}`,
    ].join(" ");
  }
...
const MainCanvas = (props) => {
  ...
  const cardDragQualifiedRef = useRef(false);
  ...
  const animateFullRotation = useCallback(() => {
    ...
    const tick = (now) => {
      ...
      if (stepRef.current === 2 && step2PhaseRef.current === "pointPlaced" && showRedArcRef.current) {
        setArcAngles((prev) => ({ ...prev, red: [rotationStartRef.current, current] }));
      }
        ...
        setArcAngles((prev) => {
          ...
          ["red", "purple", "green"].forEach((color) => {
            if (pointsRef.current[color]) {
              next[color] = [rotationStartRef.current, current];
            }
          });
          return next;
        });
  ...
  const handleCardDown = useCallback((e) => {
    ...
    cardDragQualifiedRef.current = false;
    ...
    const onMove = (moveE) => {
      ...
      const totalAngleDelta = normalizeAngleDelta(currentAngle - dragStartAngleRef.current);
      const totalPointerDistance = Math.hypot(
        movePt.x - dragStartPointRef.current.x,
        movePt.y - dragStartPointRef.current.y
      );
      if (
        !cardDragQualifiedRef.current &&
        totalPointerDistance < CARD_DRAG_MIN_DISTANCE &&
        Math.abs(totalAngleDelta) < CARD_DRAG_MIN_ANGLE
      ) {
        return;
      }
      ...
      if (stepRef.current === 2 && step2PhaseRef.current === "pointPlaced") {
        setShowRedArc(true);
        setArcAngles((prev) => ({ ...prev, red: [rotationStartRef.current, newRotation] }));
      }
      ...
      if (stepRef.current === 4) {
        ...
        setStep4ArcAngles([rotationStartRef.current, newRotation]);
      }
    };
    ...
    const onUp = () => {
      ...
      if (stepRef.current === 4) {
        step4DragStartedRef.current = false;
        if (cardDragQualifiedRef.current && typeof onStep4Drop === "function") onStep4Drop();
        return;
      }

      if (!cardDragQualifiedRef.current) {
        return;
      }
  ...
  const handleCardDownStep5 = useCallback((side, e, panelSvgRef) => {
    ...
    setStep5ClockState((prev) => ({ ...prev, [side]: "still" }));
    ...
    const onMove = (moveE) => {
      ...
      if (isLeft) {
        const newRotation = Math.max(0, rotationLeftRef.current + frameDelta);
        setRotationLeft(newRotation);
        rotationLeftRef.current = newRotation;
        setShowLeftArrows(true);
        setStep5ArcAngles((prev) => ({ ...prev, left: [rotationStartRef.current, newRotation] }));
      } else {
        const newRotation = Math.min(0, rotationRightRef.current + frameDelta);
        setRotationRight(newRotation);
        rotationRightRef.current = newRotation;
        setShowRightArrows(true);
        setStep5ArcAngles((prev) => ({ ...prev, right: [rotationStartRef.current, newRotation] }));
      }
    };
    ...
    const onUp = () => {
      ...
      if (!cardDragQualifiedRef.current) {
        setStep5ClockState((prev) => ({ ...prev, [side]: "hidden" }));
        if (isLeft) setShowLeftArrows(false);
        else setShowRightArrows(false);
        return;
      }

      const finalRotation = isLeft ? rotationLeftRef.current : rotationRightRef.current;
      const qualifies = isLeft
        ? finalRotation > STEP5_MIN_ROTATION
        : finalRotation < -STEP5_MIN_ROTATION;
      ...
      setStep5ClockState((prev) => ({ ...prev, [side]: "gif" }));
    ...
    if (sectorPath && Math.abs(panelRotation) > 0.5) {
      items.push(
        React.createElement("path", {
          key: "angle-sector",
          className: step === 4 && step4Phase === "done" ? "angle-sector-blink" : undefined,
          d: sectorPath,
          fill: SECTOR_FILL,
  ...
  }, [points.red, getRedBaseAngle, getRedPointWorld, step, step4Phase]);
  ...
  const renderStep5PanelSVG = (side, panelSvgRef) => {
    ...
    const clockState = step5ClockState[side];
    ...
    if (clockState !== "hidden") {
      elements.push(
        React.createElement("image", {
          key: "clock",
          href: clockState === "gif" ? "assets/clock.gif" : "assets/clockStill.gif",
