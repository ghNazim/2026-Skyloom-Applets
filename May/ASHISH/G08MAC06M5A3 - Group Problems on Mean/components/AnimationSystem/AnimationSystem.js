(() => {
  ...
  const useGhostFlight = (containerRef) => {
    ...
    const triggerGhost = ({
      ...
      const container =
        containerRef.current || document.querySelector(".compare-screen");
      ...
      const sourceFontPx =
        parseFloat(sourceStyle.fontSize) || sourceRect.height;
      const targetFontPx =
        parseFloat(targetStyle.fontSize) || targetRect.height;
      ...
      const newGhost = {
        id: ghostId,
        text,
        left: `${(startLeft / window.innerWidth) * 100}vw`,
        top: `${(startTop / window.innerHeight) * 100}vh`,
        width: `${(sourceRect.width / window.innerWidth) * 100}vw`,
        height: `${(sourceRect.height / window.innerHeight) * 100}vh`,
        dx: `${(dx / window.innerWidth) * 100}vw`,
        dy: `${(dy / window.innerHeight) * 100}vh`,
        scale,
        fontSize: `${(sourceFontPx / window.innerWidth) * 100}vw`,
        fontWeight: sourceStyle.fontWeight,
        lineHeight: Number.isFinite(parseFloat(sourceStyle.lineHeight))
          ? `${(parseFloat(sourceStyle.lineHeight) / window.innerHeight) * 100}vh`
          : 1,
        colorClass,
        active: false,
      };
      ...
      const raf1 = requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          setGhosts((prev) =>
            prev.map((g) => (g.id === ghostId ? { ...g, active: true } : g)),
          );
        });
      });
  ...
  const useMergeAnimation = (stageRef, targetRef) => {
    ...
    const startMerge = ({
      ...
      const chips = Array.from(units).map((unitEl) => {
        ...
        return {
          idx,
          left: `${((chipRect.left - stageRect.left) / window.innerWidth) * 100}vw`,
          top: `${((chipRect.top - stageRect.top) / window.innerHeight) * 100}vh`,
          width: `${(chipRect.width / window.innerWidth) * 100}vw`,
          height: `${(chipRect.height / window.innerHeight) * 100}vh`,
          dx: `${((targetCenterX - chipCenterX) / window.innerWidth) * 100}vw`,
          dy: `${((targetCenterY - chipCenterY) / window.innerHeight) * 100}vh`,
          isRightGroup: idx >= leftCount,
          useWhite:
            unitEl.classList.contains("merge-unit-white") ||
            unitEl.classList.contains("count-merge-unit"),
        };
      });
