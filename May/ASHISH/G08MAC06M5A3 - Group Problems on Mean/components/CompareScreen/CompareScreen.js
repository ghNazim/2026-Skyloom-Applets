const CompareScreen = ({ screen, onComplete, onInstruction, playSfx, onAnimationBusy, onStartOver, startOverButtonRef }) => {
  ...
  const title = screen.type === "builder" ? (screen.mode === "add" ? T.ui.phaseBuild : T.ui.buildSubtractTitle)
    ...
    : "";
  let content;
  if (screen.type === "builder") content = h(GroupBuilder, { mode: screen.mode, onComplete, onInstruction, onAnimationBusy, playSfx });
  if (screen.type === "rules") content = h(RuleExplorer, { onComplete, onInstruction, playSfx });
  if (screen.type === "problem") content = h(ProblemSolver, { problem: screen.problem, onComplete, onInstruction, playSfx });
