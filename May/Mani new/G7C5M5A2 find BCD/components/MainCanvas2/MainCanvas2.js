const MainCanvas2 = (function () {
  ...
  const PT_R = 8;
  const LABEL_R = 88;
  const ANGLE_LABEL_R = { A: 72, B: 88, C: 96, D: 88 };
  const VIEW_W = 800;
  const VIEW_H = 500;
  const ALL_LINES = ["k", "l", "m", "n"];
  const ALL_ANGLES = ["A", "B", "C", "D"];
  const WRONG_DEHI_B = ["m_rayA", "m_rayB", "k_rayA", "l_rayB"];
  const WRONG_DEHI_C = ["l_rayB", "l_rayC", "m_rayA", "m_rayB", "n_rayD"];
  const WRONG_DEHI_D = ["k_rayA", "m_rayA", "m_rayB", "n_rayC", "n_rayD"];
  ...
  const buildAngles = () => {
    ...
    const nExtPastC = extend(D.x, D.y, C.x, C.y, EXT, false);
    const kExtPastD = extend(A.x, A.y, D.x, D.y, EXT, false);
    const arcC = interiorArc(C.x, C.y, B, nExtPastC);
    const arcD = interiorArc(D.x, D.y, C, kExtPastD);
    return [
  ...
  return function MainCanvas2(props) {
    ...
    const [runtimeParallelBlink, setRuntimeParallelBlink] = useState([]);
    const [runtimeTransversalBlink, setRuntimeTransversalBlink] = useState([]);
    const [lineColors, setLineColors] = useState({});
    const [parallelTextKey, setParallelTextKey] = useState(null);
    const [parallelTextBlink, setParallelTextBlink] = useState(false);
    const [transversalTextVisible, setTransversalTextVisible] = useState(false);
    const [transversalTextBlink, setTransversalTextBlink] = useState(false);
    ...
    if (runtimeParallelBlink.length) vis.colorBlinkLines = runtimeParallelBlink;
    if (runtimeTransversalBlink.length) {
      vis.transversalBlinkLines = runtimeTransversalBlink;
    }
    if (parallelTextKey) vis.parallelText = parallelTextKey;

    /* ── sync canvas state to current step (incl. going back) ── */
    useEffect(() => {
      animRunRef.current++;
      if (flyAnimRef.current) {
        flyAnimRef.current.kill();
        flyAnimRef.current = null;
      }
      setFlyingValue(null);

      setAngleBLabel(step > 7 ? String(APP_DATA.angleB.correct) : null);
      setAngleCLabel(step > 9 ? String(APP_DATA.angleC.correct) : null);
      setAngleDLabel(step > 11 ? String(APP_DATA.angleD.correct) : null);

      if (!FIND_STEPS.includes(step)) {
        setBlinkFocus(null);
        setRuntimeParallelBlink([]);
        setRuntimeTransversalBlink([]);
        setLineColors({});
        setParallelTextKey(null);
        setParallelTextBlink(false);
        setTransversalTextVisible(false);
        setTransversalTextBlink(false);
      ...
      if (!MCQ_STEPS.includes(step) && onAnimatingChange) {
    ...
    useEffect(() => {
      ...
      var runAnim = async function () {
        ...
        if (onUpdateTexts) onUpdateTexts(stepData.animLines, " ");
        setParallelTextKey(cfg.parallelText);
        setParallelTextBlink(true);
        setTransversalTextVisible(true);
        setTransversalTextBlink(true);
        setRuntimeParallelBlink(cfg.parallels);
        setRuntimeTransversalBlink([cfg.transversal]);
        await wait(3000);
        if (animRunRef.current !== runId) return;
        setRuntimeParallelBlink([]);
        setRuntimeTransversalBlink([]);
        ...
        if (animRunRef.current === runId && onAnimatingChange) {
          onAnimatingChange(false);
        }
      };
    ...
    var handleOptionClick = function (option) {
      ...
      var stepKey = MCQ_STEP_KEY[step];
      var stepData = APP_DATA[stepKey];
      if (option === stepData.correctOption) {
    ...
    var flyValueToAngle = function (angleId, valueText, color, onDone) {
      ...
      if (typeof gsap !== "undefined") {
        ...
        flyAnimRef.current = gsap.to(proxy, {
          x: target.x,
          y: target.y,
          duration: 0.85,
          ease: "power2.inOut",
    ...
    var renderLines = function () {
      return h("g", null,
        LINE_KEYS.map(function (key) {
          ...
          var parallelBlink =
            includes(vis.colorBlinkLines, lineName) && !forcedDehi;
          var transversalBlink =
            includes(vis.transversalBlinkLines || [], lineName) && !forcedDehi;
          ...
          if (parallelBlink) groupClass += " color-blink-line-yellow";
          if (transversalBlink) groupClass += " color-blink-line-blue";
    ...
    var renderParallelText = function () {
      ...
      var textClass = "parallel-lines-label" +
        (parallelTextBlink ? " opacity-blink-text" : "");

      return h("text", {
        x: pos.x, y: pos.y,
        className: textClass,
        style: { fill: COL_YELLOW },
        transform: "rotate(" + pos.rotate + " " + pos.x + " " + pos.y + ")",
      }, label);
    };

    var renderTransversalText = function () {
      if (!transversalTextVisible) return null;
      var pos;
      ...
      var transClass = "transversal-label" +
        (transversalTextBlink ? " opacity-blink-text" : "");
    ...
    var renderRightPanel = function () {
      ...
      if (!MCQ_STEPS.includes(step)) return null;

      var mcqStepData = APP_DATA[MCQ_STEP_KEY[step]];
      var correctOption = mcqStepData.correctOption;
      ...
      return h("div", { className: panelClass },
        ...
        h("div", { className: "mc2-options-container" },
          APP_DATA.options.map(function (option) {
            ...
            var isCorrect = optionAnswered && option === correctOption;
