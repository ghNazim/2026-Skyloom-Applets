...
const MainCanvas = (props) => {
  ...
  useEffect(() => {
    ...
    setVisualImage(step3Data[0].img);
  ...
  useEffect(() => {
    ...
    registerActionNudge();
  }, [step, step3Substep, step3Revealed, registerActionNudge]);
  ...
  }, [advanceRef, tryAdvanceStep2, tryAdvanceStep3, tryAdvanceStep4]);

  // ── Layout widths ──
  const infoWidth = step === 1 ? "100%" : step === 2 ? "45%" : "0%";
  const visualWidth = step === 1 ? "0%" : "55%";
  const mathWidth = step === 4 ? "45%" : "0%";
  const visualMarginLeft = step === 3 ? "22.5%" : "0%";
  const visualCentering = step === 3;
        ...
        mathVisible &&
          React.createElement(
            ...
            React.createElement(
              ...
              renderFraction(
                React.createElement("span", { className: "math-h" }, "h"),
