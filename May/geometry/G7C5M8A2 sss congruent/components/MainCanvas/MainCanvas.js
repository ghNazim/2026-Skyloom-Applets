...
const FINAL_SNAP_DISTANCE = 15; // Step 3: max px gap for two endpoints to count as one vertex
...
const MainCanvas = (props) => {
  ...
  useEffect(() => {
    ...
    const handleMove = (e) => {
      ...
      setTriPoints((prev) => ({ ...prev, [dragging]: { x: newX, y: newY } }));
  ...
  }, [dragging, getSVGPoint, initialPositions, onSetNextEnabled, onUpdateTexts]);
  ...
  useEffect(() => {
    if (step === 2) {
      ...
      gsap.to(anim, {
        ...
        onComplete: () => setTriShiftX(targetShift),
      });
    }
  }, [step]);
  ...
  }, [step2ExitPending, step, onStep2FadeComplete]);
  ...
  const handleRatioClick = useCallback((ratio) => {
    ...
    const lBC = sideLen(leftPts.B, leftPts.C);
    ...
    const displayBC = fmtScaledDisplay(blueDisplayed.BC, ratio);

    const lines = [
      { id: "line-ac", length: lAC * ratio * SCALE, displayLength: displayAC, origSide: "AC" },
      { id: "line-bc", length: lBC * ratio * SCALE, displayLength: displayBC, origSide: "BC" },
    ...
    onUpdateTexts(APP_DATA.steps[2].questionAfterRatio, undefined);

    setTimeout(() => {
      runRatioBoxAnimation(ratio, lines, leftPts);
    }, 1000);
  }, [selectedRatio, triPoints, onUpdateTexts]);
  ...
  const runRatioBoxAnimation = (ratio, lines, leftPts) => {
    ...
    const sideMap = { AC: { p1: leftPts.A, p2: leftPts.C }, BC: { p1: leftPts.B, p2: leftPts.C }, AB: { p1: leftPts.A, p2: leftPts.B } };
    const fractionOrder = ["AC", "BC", "AB"];
    ...
    const doStep = () => {
      ...
      if (stepIdx > 9) {
        ...
        setTimeout(() => {
          ...
          onSetNextEnabled(true);
          onUpdateTexts(APP_DATA.steps[2].questionAfterRatio, APP_DATA.steps[2].navAfterAnimation);
        }, 500);
        return;
      }
  ...
  const markTriangleComplete = useCallback(() => {
  ...
  const doSnap = useCallback((lines) => {
    if (triangleFormedRef.current) return lines;
    const completed = tryCompleteTriangle(lines, FINAL_SNAP_DISTANCE);
    if (completed) {
      markTriangleComplete();
      return completed;
    }
    return lines;
  }, [markTriangleComplete]);

  const applySnap = useCallback((prev) => {
    if (!prev) return prev;
    return doSnap(prev);
  }, [doSnap]);
  ...
  const handleLineMoveStart = useCallback((lineIdx, e) => {
    ...
    const handleMove = (ev) => {
      ...
      const dx = p.x - ref.startMouse.x;
      const dy = p.y - ref.startMouse.y;
      setLineStates((prev) => {
        if (!prev) return prev;
        const next = [...prev];
        next[ref.lineIdx] = {
    ...
    const handleUp = () => {
      ...
      window.removeEventListener("touchmove", handleMove);
      window.removeEventListener("touchend", handleUp);
      setLineStates((prev) => prev ? applySnap(prev) : prev);
    };
    window.addEventListener("mousemove", handleMove);
    window.addEventListener("mouseup", handleUp);
    window.addEventListener("touchmove", handleMove, { passive: false });
  ...
  }, [step, lineStates, triShiftX, onUpdateTexts, onStep4Phase]);
  ...
  const handleStep5NameClick = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (nextBtn && onRegisterNudgeTarget) {
        onRegisterNudgeTarget(nextBtn.getBoundingClientRect());
      }
    }, 350);
  }, [onHideNudge, onStep5NameReveal, onRegisterNudgeTarget]);

  // ── Rendering ──
  const renderBlueTriangle = () => {
    const pts = (step === 4 || step === 5) && animBluePts ? animBluePts : currentTriPoints;
    const centroid = triCentroid(pts);
    const pathD = `M${pts.A.x} ${pts.A.y} L${pts.B.x} ${pts.B.y} L${pts.C.x} ${pts.C.y} Z`;
    const labelOffset = 28;
    const sides = [
  ...
  const renderRatioBox = () => {
    ...
    const yBC = yellowLines.find((l) => l.origSide === "BC");
    const yAB = yellowLines.find((l) => l.origSide === "AB");

    const fractions = [
      { num: yAC ? yAC.displayLength : "0", den: fmtLen(blueDisplayed.AC) },
      { num: yBC ? yBC.displayLength : "0", den: fmtLen(blueDisplayed.BC) },
    ...
    return React.createElement(
      ...
      React.createElement("text", {
        x: startX + 2 * fracSpacing + resultOffsetX, y: centerY,
        fill: COLOR_WHITE, fontSize: 18, fontWeight: 700,
        textAnchor: "middle", dominantBaseline: "middle",
        style: { opacity: vis(10), transition: "opacity 0.3s" },
      }, "= " + selectedRatio)
    );
  };
  ...
  const renderStep3Lines = () => {
    if (!lineStates || step !== 3) return null;
    const labelOffset = 28;
    const formedCentroid = triangleFormed ? getFormedTriangleCentroid(lineStates) : null;
    ...
    return React.createElement(
      ...
      lineStates.map((line, idx) => {
        ...
        const showMoveHandle = !triangleFormed && !line.locked;
        const canRotate = !triangleFormed && !line.locked;
        const leftEp = getLeftEndpointKey(line);
        const leftPt = leftEp === "p1" ? line.p1 : line.p2;
        ...
        return React.createElement(
          ...
          React.createElement("circle", {
            ...
            r: YELLOW_POINT_RADIUS, fill: COLOR_YELLOW,
            style: {
              cursor: canRotate && leftEp !== "p1" ? "grab" : "default",
              pointerEvents: canRotate && leftEp === "p1" ? "none" : "auto",
            },
            onMouseDown: canRotate && leftEp !== "p1" ? (e) => handleEndpointRotateStart(idx, "p1", e) : undefined,
            onTouchStart: canRotate && leftEp !== "p1" ? (e) => handleEndpointRotateStart(idx, "p1", e) : undefined,
          }),
          React.createElement("circle", {
            cx: line.p2.x, cy: line.p2.y,
            r: YELLOW_POINT_RADIUS, fill: COLOR_YELLOW,
            style: {
              cursor: canRotate && leftEp !== "p2" ? "grab" : "default",
              pointerEvents: canRotate && leftEp === "p2" ? "none" : "auto",
            },
            onMouseDown: canRotate && leftEp !== "p2" ? (e) => handleEndpointRotateStart(idx, "p2", e) : undefined,
            onTouchStart: canRotate && leftEp !== "p2" ? (e) => handleEndpointRotateStart(idx, "p2", e) : undefined,
          }),
          canRotate && React.createElement("image", {
            href: ROT_HANDLE_SRC,
            x: leftPt.x - ROT_HANDLE_SIZE / 2,
            y: leftPt.y - ROT_HANDLE_SIZE / 2,
            width: ROT_HANDLE_SIZE,
            height: ROT_HANDLE_SIZE,
            className: "rot-handle-image",
            style: { cursor: "grab", pointerEvents: "all" },
            onMouseDown: (e) => handleEndpointRotateStart(idx, leftEp, e),
            onTouchStart: (e) => handleEndpointRotateStart(idx, leftEp, e),
          }),
  ...
  const renderOverlapYellow = () => {
    if ((step !== 4 && step !== 5) || !animYellow) return null;
    const { X, Y, Z, sides } = animYellow;
    const labelInside = step === 5 ? 1 : animYellow.labelInside;
    const yellowPts = { A: X, B: Y, C: Z };
    const centroid = triCentroid(yellowPts);
    const labelOffset = 28;
    const pathD = `M${X.x} ${X.y} L${Y.x} ${Y.y} L${Z.x} ${Z.y} Z`;
    ...
    const blueLens = animBluePts ? getBlueDisplayedLengths(animBluePts) : null;

    const displayLen = (key, yellowVal) => {
      if (!blueLens || labelInside <= 0) return yellowVal;
      const blueVal = parseFloat(fmtLen(blueLens[key]));
      const yellowNum = parseFloat(yellowVal);
      const blended = yellowNum + (blueVal - yellowNum) * labelInside;
      return fmtLen(blended);
    };
    ...
    return React.createElement(
      ...
      yellowSides.map((side) => {
        ...
        return React.createElement(
          ...
          displayLen(side.key, side.yellowLen) + " cm",
        );
      }),
    );
  };
  ...
  const renderStep4Overlays = () => {
    ...
    return React.createElement(
      ...
      step4BottomVisible &&
        React.createElement(
          "div",
          { className: "bottom-text panel-fade-in" },
          stepData.bottomText,
        ),
      step4ConcludeVisible &&
        React.createElement(
          "button",
          {
            id: "conclude-button",
