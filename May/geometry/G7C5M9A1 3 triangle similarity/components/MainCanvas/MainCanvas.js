...
const MainCanvas = (props) => {
  ...
  const runStep4Animation = useCallback(async () => {
    ...
    onUpdateTexts(undefined, APP_DATA.steps[4].navAfterAnim);
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(() => runStep4Animation(), 500);
  ...
  const runStep8AdbAnimation = useCallback(async () => {
    ...
    const final = { ax: A.x, ay: A.y, dx: B.x, dy: B.y, bx: C.x, by: C.y };
    ...
    setMathVisible(true);
    onUpdateTexts(undefined, "Activity completed!! Tap start over to restart");
    if (onSetNextLabel) onSetNextLabel("Start Over");
    onSetNextEnabled(true);

    isAnimatingRef.current = false;
  }, [onSetNextEnabled, onSetNextLabel, onUpdateTexts]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!cancelled) onSetNextEnabled(true);
    };
    run();
    return () => {
      cancelled = true;
  ...
  const runStep3SquareClick = useCallback(async () => {
    ...
    const floatAnim = { x: floatOffsetX };
    await new Promise((resolve) => {
      gsap.to(floatAnim, {
        x: 0,
        duration: 0.7,
        ...
        isStep8OnlyMath &&
          React.createElement(
            ...
            React.createElement(
              ...
              "All three triangles fit into one another. All three are similar triangles.",
            ),
            React.createElement(
              "div",
