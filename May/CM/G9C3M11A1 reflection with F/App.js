const App = () => {
  ...
  const handleLottieUnmount = useCallback((controller) => {
    if (lottieRef.current === controller) {
  ...
  const getFreshStepMarker = useCallback((step) => {
    if (step === 1) return SCENE.A;
    if (step === 2) return SCENE.C;
    if (step === 3 || step === 4) return SCENE.D;
    if (step === 5) return SCENE.E;
    if (step === 6) return SCENE.F;
    if (step === 7) return SCENE.G;
    if (step === 8) return SCENE.K;
    return SCENE.A;
  }, []);

  const goToStepFresh = useCallback(
    (step) => {
      if (lottieRef.current) {
        lottieRef.current.cancelAnimation();
        lottieRef.current.goToMarker(getFreshStepMarker(step));
      }

      setIsAnimating(false);
      setStep1IntroDone(step !== 1);
      step1IntroStartedRef.current = false;
      setStep7Tapped(false);
      setStep7PlayingK(false);
      setShowReplayE(step === 5);
      setShowReplayF(step === 6);
      setCurrentStep(step);
    },
    [getFreshStepMarker],
  );
  ...
  const isPrevDisabled = useMemo(() => {
    return isAnimating || currentStep <= 1;
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(Navigation, {
        onNav: (dir) => (dir === "next" ? handleNext() : handlePrevious()),
