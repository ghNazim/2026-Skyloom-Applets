...
const MainCanvas = ({
  ...
  const runFindAnimation = useCallback(
    async (primeKey) => {
      ...
      await mergeResultBox(() => setRuleState(GENERIC_RULE));
      if (isCancelled()) return;
      await delay(200);
      if (isCancelled()) return;
  ...
  useEffect(() => {
    ...
    if (step !== 4) step4InitRef.current = false;
    if (step !== 5) step5InitRef.current = false;
    if (step !== 6) {
      step6InitRef.current = false;
    }
  }, [
