const MainCanvas2 = (function () {
  ...
  return function MainCanvas2(props) {
    ...
    const [mcqAnimationCompleted, setMcqAnimationCompleted] = useState(false);

    const svgRef = useRef(null);
    const inputBoxRef = useRef(null);
    const animRunRef = useRef(0);
    const DIAGRAM_RECENTER_MS = 650;
    const replayableStep = MCQ_STEPS.includes(step) || NUMPAD_STEPS.includes(step);
    const showReplayButton =
      ...
      !replayAnimating &&
      (
        (MCQ_STEPS.includes(step) && mcqAnimationCompleted && !optionAnswered) ||
        (NUMPAD_STEPS.includes(step) && !numpadDisabled)
      );

    var applySolvedAngleLabelsForStep = function (activeStep) {
      setAngleBLabel(activeStep >= 7 ? String(APP_DATA.angleB.correct) : null);
      setAngleCLabel(activeStep >= 9 ? String(APP_DATA.angleC.correct) : null);
      setAngleDLabel(activeStep >= 11 ? String(APP_DATA.angleD.correct) : null);
    };

    var getSequenceStep = function (activeStep) {
      if (MCQ_STEPS.includes(activeStep)) return activeStep;
      if (NUMPAD_STEPS.includes(activeStep)) return activeStep - 1;
      return null;
    };
    ...
    useEffect(() => {
      ...
      setMcqAnimationCompleted(false);
      if (onMcqStepBusyChange) onMcqStepBusyChange(false);
      if (step < 7) setAngleBLabel(null);
      if (step < 9) setAngleCLabel(null);
      if (step < 11) setAngleDLabel(null);
    }, [step, onMcqStepBusyChange]);
    ...
    var runSequence = async function (opts) {
      var sequenceStep = opts.sequenceStep;
      var initialQuestion = opts.initialQuestion;
      var initialNav = opts.initialNav;
      var restoreQuestion = opts.restoreQuestion;
      var restoreNav = opts.restoreNav;
      var keepPanel = !!opts.keepPanel;
      var finalPanelMode = opts.finalPanelMode;
      var finalPanelVisible = opts.finalPanelVisible;
      var finalDiagramShifted = opts.finalDiagramShifted;
      var resetMcqState = !!opts.resetMcqState;
      var cfg = MCQ_CFG[sequenceStep];
      var stepKey = MCQ_STEP_KEY[sequenceStep];
      var stepData = APP_DATA[stepKey];
      var runId = ++animRunRef.current;

      setReplayAnimating(true);
      if (MCQ_STEPS.includes(sequenceStep) && onMcqStepBusyChange) {
        onMcqStepBusyChange(true);
      }
      setBlinkFocus(null);
      setRuntimeColorBlink([]);
      setLineColors({});
      ...
      if (onUpdateTexts) onUpdateTexts(initialQuestion, initialNav);
      ...
      if (onUpdateTexts) onUpdateTexts(restoreQuestion, restoreNav);
      if (finalPanelMode !== undefined) setPanelMode(finalPanelMode);
      if (finalPanelVisible !== undefined) setPanelVisible(finalPanelVisible);
      if (finalDiagramShifted !== undefined) setDiagramShifted(finalDiagramShifted);
      setReplayAnimating(false);
      if (MCQ_STEPS.includes(sequenceStep)) {
        setMcqAnimationCompleted(true);
        if (onMcqStepBusyChange) onMcqStepBusyChange(false);
      }
    };
    ...
    useEffect(() => {
      ...
      if (checkpointViewMode === "mcq-answered") return;
      setMcqAnimationCompleted(false);
      if (onSetNextEnabled) onSetNextEnabled(false);

      runSequence({
        sequenceStep: step,
        initialQuestion: APP_DATA.steps[step - 1].questionText,
        initialNav: APP_DATA.steps[step - 1].navText,
        restoreQuestion: APP_DATA[MCQ_STEP_KEY[step]].optionsQuestion,
        restoreNav: APP_DATA.navTapCorrectOption,
        keepPanel: false,
        finalPanelMode: "options",
        finalPanelVisible: true,
        finalDiagramShifted: true,
        resetMcqState: true,
      });
    }, [step, checkpointViewMode]);
    ...
    useEffect(() => {
      ...
      if (checkpointViewMode === "answered") return;
      if (
        (step === 7 && clickableAngle === "C") ||
        (step === 9 && clickableAngle === "D")
      ) {
      ...
      if (onSetNextEnabled) onSetNextEnabled(false);
    }, [step, clickableAngle, checkpointViewMode]);

    /* ── restored answered checkpoint state for mcq steps ── */
    useEffect(() => {
      if (!MCQ_STEPS.includes(step) || checkpointViewMode !== "mcq-answered") return;
      var cfg = MCQ_CFG[step];
      var stepData = APP_DATA[MCQ_STEP_KEY[step]];
      ...
      setLineColors(cfg.lineColors);
      setMcqAnimationCompleted(false);
      applySolvedAngleLabelsForStep(step);

      if (onMcqStepBusyChange) onMcqStepBusyChange(false);
      if (onSetNextEnabled) onSetNextEnabled(true);
    }, [step, checkpointViewMode, onMcqStepBusyChange]);

    /* ── restored answered checkpoint state for numpad steps ── */
    useEffect(() => {
      if (!NUMPAD_STEPS.includes(step) || checkpointViewMode !== "numpad-answered") return;
      var lineCfg = getMcqCfgForNumpad(step);
      var angleKey = NUMPAD_ANGLE_KEY[step];
      var correctValue = String(APP_DATA[angleKey].correct);
    ...
    var handleReplay = function () {
      var sequenceStep = getSequenceStep(step);
      if (!sequenceStep || replayAnimating) return;
      playSnd("click");
      runSequence({
        sequenceStep: sequenceStep,
        initialQuestion: APP_DATA.steps[sequenceStep - 1].questionText,
        initialNav: APP_DATA.steps[sequenceStep - 1].navText,
        restoreQuestion: currentQuestionText,
        restoreNav: currentNavText,
        keepPanel: true,
        finalPanelMode: panelMode,
        finalPanelVisible: panelVisible,
        finalDiagramShifted: diagramShifted,
        resetMcqState: false,
      });
    };

    var handleOptionClick = function (option) {
      if (replayAnimating || optionAnswered || !MCQ_STEPS.includes(step)) return;
      var cfg = MCQ_CFG[step];
      ...
      if (panelMode === "numpad") {
        ...
        return h("div", { className: panelClass },
          ...
          h("div", { className: "numpad-wrapper" },
            h(Numpad, {
              disabled: numpadDisabled || replayAnimating,
              onNumberClick: function (num) {
                if (numpadDisabled || replayAnimating) return;
                if (inputState === "wrong") {
                  setInputState("");
                  setShowHint(false);
                  setInputValue(num);
        ...
        h("div", { className: "mc2-options-container" },
          APP_DATA.options.map(function (option) {
            ...
            return h("button", {
              ...
              disabled: optionAnswered || replayAnimating,
    ...
    return h("div", { className: "main-canvas2-container" },
      h("div", { className: diagramClass },
        ...
        showReplayButton && h("button", {
          type: "button",
          className: "mc2-replay-button",
          onClick: handleReplay,
          disabled: replayAnimating,
        }, APP_DATA.replayText),
      ),
      renderRightPanel(),
      flyingValue && h("div", {
