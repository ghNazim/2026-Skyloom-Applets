...
const SLIDER_SNAP_THRESHOLD = 0.08;
const SLIDER_EDGE_SNAP_THRESHOLD = 0.06;

function pctFromK(k, min, max) {
  return ((k - min) / (max - min)) * 100;
}

function kFromPct(pct, min, max) {
  return min + (pct / 100) * (max - min);
}

function snapKToTarget(val, target, threshold) {
  const t = threshold !== undefined ? threshold : SLIDER_SNAP_THRESHOLD;
  return Math.abs(val - target) <= t ? target : val;
}

function snapKNearEdge(val, min, max, threshold) {
  const t = threshold !== undefined ? threshold : SLIDER_EDGE_SNAP_THRESHOLD;
  if (Math.abs(val - min) <= t) return min;
  if (Math.abs(val - max) <= t) return max;
  return val;
}

function clampSliderK(val, min, max, mode, center) {
  let v = parseFloat(val);
  const c = center !== undefined ? center : (min + max) / 2;
  if (isNaN(v)) v = c;
  v = Math.max(min, Math.min(max, v));
  if (mode === "positive") v = Math.max(c, v);
  if (mode === "negative") v = Math.min(c, v);
  return v;
}
...
function renderRichHtml(html) {
  if (!html) return "";
  return html
    .replace(/<mk>([\s\S]*?)<\/mk>/g, '<span class="math-k">$1</span>')
    .replace(/<g>([\s\S]*?)<\/g>/g, '<span class="rich-green">$1</span>')
    .replace(/<y>([\s\S]*?)<\/y>/g, "<y>$1</y>");
}

function animateKWithGsap(fromK, toK, duration, onUpdate, onComplete) {
  const state = { k: fromK };
  const tween = gsap.to(state, {
    ...
    ease: "power2.inOut",
    onUpdate: function () {
      if (typeof onUpdate === "function") onUpdate(state.k);
    },
    onComplete: function () {
...
const DILATION_ANIM_DURATION = 1400;
