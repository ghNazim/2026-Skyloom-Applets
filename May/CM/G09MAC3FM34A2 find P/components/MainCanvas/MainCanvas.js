...
/** Pick √13 / √17 / √8 from actual segment length (avoids AB↔AC swap after reflection). */
function rootValueForSegment(p1, p2) {
  if (!p1 || !p2) return SIDE_LENGTH_CONFIG.AB.rootValue();
  const d2 = Math.round(
    (p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y),
  );
  if (Math.abs(d2 - 13) <= 1) return SIDE_LENGTH_CONFIG.AC.rootValue();
  if (Math.abs(d2 - 8) <= 1) return SIDE_LENGTH_CONFIG.BC.rootValue();
  return SIDE_LENGTH_CONFIG.AB.rootValue();
}
...
const MainCanvas = ({
  ...
  const eqCloneLetterRefs = useRef({ B: null, C: null });
  ...
  const [step7OrVisible, setStep7OrVisible] = useState(false);
  const [step7Eq2Visible, setStep7Eq2Visible] = useState(false);
  const [step7BelowVisible, setStep7BelowVisible] = useState(false);
  const [step7PromptVisible, setStep7PromptVisible] = useState(true);
  const [eqRowClone, setEqRowClone] = useState(null);
  ...
  const [highlightTranslatedP, setHighlightTranslatedP] = useState(null); // "PQR1" | "PQR1,PQR2" | "PQR3" | "PQR3,PQR4" | null

  const isPHighlighted = useCallback(
    (key) => {
      if (!highlightTranslatedP) return false;
      return ("," + highlightTranslatedP + ",").indexOf("," + key + ",") !== -1;
    },
    [highlightTranslatedP],
  );

  const addPHighlight = useCallback((key) => {
    setHighlightTranslatedP((prev) => {
      if (!prev) return key;
  ...
  const [sidePanelBelowVisible, setSidePanelBelowVisible] = useState(false);
  const [sidePanelBelowOpacity, setSidePanelBelowOpacity] = useState(0);
  ...
  const buildQrGraph = useCallback(() => {
    ...
    setPointRevealState({
      ...
      Q: { circleOpacity: 1, labelOpacity: 1 },
      R: { circleOpacity: 1, labelOpacity: 1 },
      P: { circleOpacity: 0, labelOpacity: 0 },
    });
    setShowAbcTriangle(true);
    setShowQrSegment(true);
    setShowUnknownPoint(false);
  ...
  const runStep7Correct = useCallback(async () => {
    ...
    setStep7OrVisible(false);
    setStep7Eq2Visible(false);
    setStep7BelowVisible(false);
    setEqRowClone(null);
    await delay(360);
    if (isCancelled()) return;
    ...
    // 1) Top eq text formed
    setStep7Reveal(true);
    await delay(600);
    if (isCancelled()) return;

    // 2) Bring OR row
    setStep7OrVisible(true);
    await delay(600);
    if (isCancelled()) return;

    for (let i = 0; i < 12 && (!eq1Ref.current || !eq2Ref.current); i++) {
      await delay(30);
      if (isCancelled()) return;
    }
    const src = eq1Ref.current;
    const tgt = eq2Ref.current;
    if (!src || !tgt) {
      setStep7Eq2Visible(true);
      setStep7BelowVisible(true);
    ...
    const dx = tgtRect.left + tgtRect.width / 2 - startX;
    const dy = tgtRect.top + tgtRect.height / 2 - startY;

    // 3) Clone down to form bottom eq row
    setEqRowClone({
      startX: startX,
      startY: startY,
      dx: dx,
    ...
    setEqRowClone((prev) => (prev ? { ...prev, animating: true } : prev));
    await delay(700);
    if (isCancelled()) return;

    // 4) Pause, then swap B and C
    await delay(600);
    if (isCancelled()) return;

    const bEl = eqCloneLetterRefs.current.B;
    const cEl = eqCloneLetterRefs.current.C;
    let swapDx = 0;
    if (bEl && cEl) {
      const bRect = bEl.getBoundingClientRect();
      const cRect = cEl.getBoundingClientRect();
      swapDx = cRect.left + cRect.width / 2 - (bRect.left + bRect.width / 2);
    }
    setEqRowClone((prev) =>
      prev ? { ...prev, swapping: true, swapDx: swapDx } : prev,
    );
    await delay(550);
    if (isCancelled()) return;
  ...
  const resetStep12Local = useCallback(() => {
    ...
    setSidePanelBelowVisible(false);
    setSidePanelBelowOpacity(0);
  }, []);
  ...
  useEffect(() => {
    ...
    if (step === 11) {
      ...
      resetStep15Local();
      resetStep16Local();
      // Keep PQR1 highlight from translate through the reflect step
      setHighlightTranslatedP((prev) => {
        if (!prev) return "PQR1";
        if (("," + prev + ",").indexOf(",PQR1,") !== -1) return prev;
        return "PQR1";
      });
      setRotationLocked(true);
      setShowRotationClone(false);
    ...
    if (step === 12 || step === 13) {
      ...
      if (step === 12) {
        resetStep13Local();
        if (step12Phase === "done") {
          setSidePanelBelowVisible(true);
          setSidePanelBelowOpacity(1);
    ...
    if (step >= 14) {
      ...
      if (step === 16) {
        ...
        setShowSecondRotationClone(false);
        setSecondTranslateLocked(true);
        // Keep PQR3 highlight from translate through the reflect step
        setHighlightTranslatedP((prev) => {
          if (!prev) return "PQR3";
          if (("," + prev + ",").indexOf(",PQR3,") !== -1) return prev;
          return "PQR3";
        });
        setShowPqr3(true);
        setPqr3Coords((prev) => prev || buildDefaultPqr3());
  ...
  const handleStep9SliderCommit = () => {
    ...
    // Step 9: only 90° CW or 270° ACW (case A)
    const isCorrect =
      (rotationDirection === "cw" && snapped === 90) ||
      (rotationDirection === "acw" && snapped === 270);
  ...
  const handleStep14SliderCommit = () => {
    ...
    // Step 14: only 90° ACW or 270° CW (case B)
    const isCorrect =
      (secondRotationDirection === "acw" && snapped === 90) ||
      (secondRotationDirection === "cw" && snapped === 270);
  ...
  const graphPoints = useMemo(() => {
    ...
    if (showPqr1 && pqr1Coords) {
      ...
      const pHighlight = isPHighlighted("PQR1");
      const pColor = pHighlight ? "#ffd23f" : colors.image;
      points.push(
        {
          id: "PQR1-P",
          x: P.x,
    ...
    if (pqr2Display) {
      ...
      const pDim = dimFirstPqr ? FIRST_PQR_DIM : 1;
      const pHighlight = isPHighlighted("PQR2");
      points.push({
        id: "PQR2-P",
        x: P.x,
        y: P.y,
        color: pHighlight ? "#ffd23f" : colors.image,
    ...
    if (showPqr3 && pqr3Coords) {
      ...
      const pHighlight = isPHighlighted("PQR3");
      points.push({
        id: "PQR3-P",
        x: P.x,
        y: P.y,
    ...
    if (pqr4Display) {
      ...
      const pHighlight = isPHighlighted("PQR4");
      points.push({
        id: "PQR4-P",
        x: P.x,
        y: P.y,
  ...
  const graphSegments = useMemo(() => {
    ...
    if (showUnknownPoint && (!showPqr1 || isOriginalPqrExiting)) {
      const pReveal =
        pointRevealState.P && pointRevealState.P.circleOpacity != null
          ? pointRevealState.P.circleOpacity
          : 1;
      const dashOpacity = isOriginalPqrExiting
        ? 0.68 * originalPqrFadeOpacity * pReveal
        : 0.68 * pReveal;
      segments.push(
        {
          from: graph.coords.Q,
  ...
  if (pqr2Display && reflectProgress > 0.82) {
    const P = pqr2Display.finalP || pqr2Display.P;
    const Q = pqr2Display.Q;
    const R = pqr2Display.R;
    const tri = { A: P, B: Q, C: R };
    const positions = getOutwardSideLabelPositions(tri.A, tri.B, tri.C, 0.7);
    const showNonQr = !dimFirstPqr;
    rootLabels.push(
      {
  ...
  if (pqr4Display && secondReflectProgress > 0.82) {
    const P = pqr4Display.finalP || pqr4Display.P;
    const Q = pqr4Display.Q;
    const R = pqr4Display.R;
    const tri = { A: P, B: Q, C: R };
    const positions = getOutwardSideLabelPositions(tri.A, tri.B, tri.C, 0.7);
    rootLabels.push(
      {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              "mtext",
              {
                className: "math-slot" + (slots.x2 ? "" : " is-var"),
                ref: (el) => {
            ...
            React.createElement(
              "mtext",
              {
                className: "math-slot" + (slots.x1 ? "" : " is-var"),
                ref: (el) => {
            ...
            React.createElement(
              "mtext",
              {
                className: "math-slot" + (slots.y2 ? "" : " is-var"),
                ref: (el) => {
            ...
            React.createElement(
              "mtext",
              {
                className: "math-slot" + (slots.y1 ? "" : " is-var"),
                ref: (el) => {
  ...
  const renderEqRow = (options) => {
    const isEq1 = options.variant === "eq1";
    const rightFirst = isEq1 ? "B" : "C";
    const rightSecond = isEq1 ? "C" : "B";
    return React.createElement(
      "div",
      {
        className: "step7-eq-text" + (options.className ? " " + options.className : ""),
        ref: options.containerRef || null,
        style: { opacity: options.opacity },
        "aria-hidden": options.opacity < 0.05,
      },
      React.createElement(
        "span",
        {
  ...
  const renderEqBlock = () => {
    const highlight = step >= 12 ? "eq2" : step >= 9 ? "eq1" : "both";
    const dim = 0.32;
    const eq1Opacity =
      step === 7 && !step7Reveal ? 0 : highlight === "eq2" ? dim : 1;
    const eq2Opacity =
      step === 7 && !step7Eq2Visible
        ? 0
        : highlight === "eq1"
          ? dim
          : 1;
    const orOpacity =
      step === 7 && !step7OrVisible ? 0 : highlight === "both" ? 1 : dim;

    return React.createElement(
      "div",
      { className: "step7-eq-stack" },
      renderEqRow({
        variant: "eq1",
        opacity: eq1Opacity,
        containerRef: (el) => {
        ...
        className: "step7-eq-text-1",
      }),
      React.createElement(
        "div",
        {
          className: "step7-eq-or",
          style: { opacity: orOpacity },
        },
        APP_DATA.steps[7].orLabel || "OR",
      ),
      renderEqRow({
        variant: "eq2",
        opacity: eq2Opacity,
        ...
        className: "step7-eq-text-2",
      }),
    );
  };

  const renderEqRowClone = () => {
    if (!eqRowClone) return null;
    const swapDx = eqRowClone.swapping ? eqRowClone.swapDx || 0 : 0;
    return React.createElement(
      "div",
      {
        className:
          "step7-eq-fly-clone" + (eqRowClone.swapping ? " is-swapping" : ""),
        style: {
          left: eqRowClone.startX + "px",
          top: eqRowClone.startY + "px",
          transform: eqRowClone.animating
            ? "translate(calc(-50% + " +
              eqRowClone.dx +
              "px), calc(-50% + " +
              eqRowClone.dy +
      ...
      React.createElement(
        "span",
        {
          className: "step7-eq-letter is-orange",
          ref: (el) => {
            eqCloneLetterRefs.current.B = el;
          },
          style: { transform: "translateX(" + swapDx + "px)" },
        },
        "B",
      ),
      React.createElement(
        "span",
        {
          className: "step7-eq-letter is-orange",
          ref: (el) => {
            eqCloneLetterRefs.current.C = el;
          },
          style: { transform: "translateX(" + -swapDx + "px)" },
        },
        "C",
  ...
  const renderStep7ResultPanel = () =>
    React.createElement(
      ...
      React.createElement(
        ...
        {
          className:
            "step7-below-space" + (step7BelowVisible ? " is-ready" : ""),
        },
          ...
          ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  "span",
                  null,
                  renderMathVars(APP_DATA.steps[11].resultPPrefix),
                ),
                React.createElement(
                  "span",
                  {
          ...
          ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  "span",
                  null,
                  renderMathVars(APP_DATA.steps[16].resultPPrefix),
                ),
                React.createElement(
                  "span",
                  {
  ...
  const renderStep12Or13Panel = () => {
    ...
    return React.createElement(
      ...
      renderEqBlock(),
      sidePanelBelowVisible
        ? React.createElement(
            "div",
            {
              className:
                "step7-below-space is-step8-visible step12-below-space",
              style: { opacity: sidePanelBelowOpacity },
            },
            belowText,
          )
        : null,
