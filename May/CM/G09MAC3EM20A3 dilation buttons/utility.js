...
function renderStyledVariableText(text) {
  if (!text || text.indexOf("k") === -1) return text;
  const parts = text.split(/(\(k\))/g);
  return parts.map((part, index) =>
    part === "(k)"
      ? React.createElement(
          React.Fragment,
          { key: "math-k-group-" + index },
          "(",
          React.createElement(
            "span",
...
function renderFeedbackHtml(html) {
  if (!html) return "";
  return html
    .replace(/\bk\b/g, '<span class="math-variable">k</span>')
    .replace(
      /<hl>([\s\S]*?)<\/hl>/g,
    ...
    .replace(
      /<y>([\s\S]*?)<\/y>/g,
      '<span class="feedback-question">$1</span>',
    )
...
const SHAPE_PRE_ANIM_DELAY = 300;
const SHAPE_DILATION_ANIM_DURATION = Math.round(DILATION_ANIM_DURATION * 1.3);
const SHAPE_FEEDBACK_PAUSE_DURATION = 1000;
