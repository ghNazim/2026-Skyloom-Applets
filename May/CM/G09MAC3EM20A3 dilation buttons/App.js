const App = () => {
  ...
  const [dehighlightedPolygons, setDehighlightedPolygons] = useState([]);
  ...
  const pauseAfterShapeFeedback = useCallback((onComplete) => {
    setFeedbackHtml(
      renderFeedbackHtml(handleComma(APP_DATA.feedback.shape)),
    );
    setShapePhase("feedbackPause");
    setIsAnimating(true);

    const cleanup = delay(SHAPE_FEEDBACK_PAUSE_DURATION, () => {
      cancelRef.current = null;
      if (typeof onComplete === "function") onComplete();
      setIsAnimating(false);
    });
  ...
  const runShapePolygonAnim = useCallback(
    (shapeId, targetK, points, dehighlightIds, onComplete) => {
      cancelAnimations();
      setIsAnimating(true);
      ...
      setRayProgress(0);
      setFeedbackHtml("");
      setDehighlightedPolygons(dehighlightIds || []);
      ...
      const cleanups = [];
      cleanups.push(
        delay(SHAPE_PRE_ANIM_DELAY, () => {
          cleanups.push(
            animateValue(0, 1, RAY_GROW_DURATION, setRayProgress, () => {
              setShapeDil(true);
              playDilationSweep(SHAPE_DILATION_ANIM_DURATION);
              cleanups.push(
                animateValue(
                  1,
                  targetK,
                  SHAPE_DILATION_ANIM_DURATION,
                  setShapeK,
                  () => {
                    setShapeK(targetK);
  ...
  }, [shapePhase, isAnimating, runShapePolygonAnim, pauseAfterShapeFeedback]);
  ...
  const polygons = useMemo(() => {
    ...
    if (triangleVisible) {
      list.push({
        ...
        dilationK: dilationK,
        dimmed: dehighlightedPolygons.includes("triangle"),
      });
    }
    ...
    if (activeProperty === "shape" && showSquare) {
      list.push({
        ...
        dilationK: squareK,
        dimmed: dehighlightedPolygons.includes("square"),
    ...
    if (activeProperty === "shape" && showTrapezium) {
      list.push({
        ...
        dimmed: dehighlightedPolygons.includes("trapezium"),
  ...
  const feedbackPromptHtml = useMemo(() => {
    if (
      activeProperty === "orientation" &&
      (orientationPhase === "interactive" || orientationPhase === "rotate")
    ) {
      return renderFeedbackHtml(
        handleComma(APP_DATA.feedback.orientationQuestion),
      );
    }
    return "";
  }, [activeProperty, orientationPhase]);
  ...
  const graphProps = {
    ...
    feedbackPromptHtml: feedbackPromptHtml,
  };
