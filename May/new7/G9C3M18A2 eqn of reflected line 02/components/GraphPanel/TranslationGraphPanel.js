...
const REFLECTION_PROBLEM_GRAPH_CONFIGS = {
  q1: {
    cols: 16,
    rows: 10,
    unit: 44,
...
const TranslationGraphPanel = ({
  ...
  const DIMMED_AXIS_COLOR = "rgba(255, 255, 255, 0.4)";
  const BG_COLOR = "none";
  ...
  const renderExtendedLine = (line, index) => {
    ...
    const equationLabelEl = line.equationLabel
      ? (function () {
          const labelRatio =
            line.labelPositionRatio != null ? line.labelPositionRatio : 0.5;
          const labelBaseFrom = line.labelAtThroughMidpoint
            ? line.through[0]
            : bounds.from;
          const labelBaseTo = line.labelAtThroughMidpoint
            ? line.through[1]
            : bounds.to;
          const labelGraphX =
            labelBaseFrom.x + (labelBaseTo.x - labelBaseFrom.x) * labelRatio;
          const labelGraphY =
            labelBaseFrom.y + (labelBaseTo.y - labelBaseFrom.y) * labelRatio;
          const labelPos = toSvg(labelGraphX, labelGraphY);
          const angleRad = Math.atan2(to.y - from.y, to.x - from.x);
          const angleDeg =
            (angleRad * 180) / Math.PI +
            180 +
            (line.labelAngleOffset != null ? line.labelAngleOffset : 0);
          const perpOffset =
          ...
          return React.createElement(
            ...
            buildMathVariableSvgChildren(line.equationLabel, React.createElement),
          );
        })()
      : null;
  ...
  const reflectionAxisLineEl = reflectionAxisLine
    ...
    : null;
  const xAxisColor = DIMMED_AXIS_COLOR;
  const yAxisColor = yAxisHighlight ? REFLECTION_GRAPH_ORANGE : DIMMED_AXIS_COLOR;
