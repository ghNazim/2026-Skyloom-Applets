...
const audioCache = {};
const sounds = ["correct", "wrong", "click", "congrats", "tick", "swoosh", "zoom"];
sounds.forEach((name) => {
...
function formatMathVariables(text) {
  if (!text) return text;
  return String(text).replace(
    /(?<![a-zA-Z])(x|y)(?=-axis|(?![a-zA-Z-]))/g,
    function (match) {
      return "<span class=\"math-var\">" + match + "</span>";
    },
  );
}

function formatMathVariablesInHtml(html) {
  if (!html) return html;
  return String(html).replace(/(<[^>]*>)|([^<]+)/g, function (match, tag, text) {
    if (tag) return tag;
    if (text) return formatMathVariables(text);
    return match;
  });
}

function buildMathVariableSvgChildren(text, createElement) {
  if (!text) return [""];
  var parts = [];
  var regex = /(?<![a-zA-Z])(x|y)(?=-axis|(?![a-zA-Z-]))/g;
  var lastIndex = 0;
  var match;
  var key = 0;
