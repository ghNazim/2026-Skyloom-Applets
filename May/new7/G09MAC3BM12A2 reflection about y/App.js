const App = () => {
  ...
  const [unitLineY1, setUnitLineY1] = useState(5);
  const [unitLineY2, setUnitLineY2] = useState(4);
  ...
  const runDistanceAnimation = useCallback(() => {
    ...
    schedule(() => {
      ...
      schedule(() => {
        afterRotate(4, 3, s4.unitPlural.replace("{n}", "2"), () => {
          setUnitLineRotating(true);
          schedule(() => {
            afterRotate(3, 2, s4.unitPlural.replace("{n}", "3"), () => {
              setUnitLineRotating(true);
              schedule(() => {
                afterRotate(2, 1, s4.unitPlural.replace("{n}", "4"), () => {
                  setUnitLineRotating(true);
                  schedule(() => {
                    afterRotate(1, 0, s4.unitPlural.replace("{n}", "5"), () => {
                      schedule(() => {
                        setShowUnitLine(false);
                        setUnitLabelFinal(true);
                        setUnitLabelText(s4.unitPlural.replace("{n}", "5"));
  ...
  const runProp1Sequence = useCallback(() => {
    ...
    const FIRST_LOOP_OSCILLATION_POINTS = [5, 0, -7, -4, -5];
    const REPEATING_OSCILLATION_POINTS = [-8,0, -7, -4, -5];
            ...
            afterRotate(-2, -3, s4.unitPlural.replace("{n}", "3"), 3, () => {
              ...
              schedule(() => {
                afterRotate(-3, -4, s4.unitPlural.replace("{n}", "4"), 4, () => {
                  setUnitLineRotating(true);
                  schedule(() => {
                    afterRotate(-4, -5, s4.unitPlural.replace("{n}", "5"), 5, () => {
                      schedule(() => {
                        setShowUnitLine(false);
                        setUnitLabelText(s4.unitPlural.replace("{n}", "5"));
                        ...
                        schedule(() => {
                          setUnitLabelOverride({ x: -2.5, y: 4 });
                        }, LABEL_MOVE_DELAY_MS);
    ...
    (value) => {
      ...
      if (step9Part === "x") {
        ...
        if (isCorrect) {
          ...
          setStep9XFeedback(
            value === question.x ? s9.xWrongNeg3 : s9.xWrongOther,
          );
        }
        return;
      }
      ...
      if (isCorrect) {
        ...
        if (value === question.x || value === question.imageX) setStep9YFeedback(s9.yWrong3);
        else if (Number(value) === -Number(question.y) && Number(question.y) !== 0) setStep9YFeedback(s9.yWrong4);
        else setStep9YFeedback(s9.yWrongNeg3);
      }
    },
    [currentStep, step9Part, step9QuestionIndex, step9XStatus, step9YStatus, schedule],
  );
  ...
  const navText = useMemo(() => {
    ...
    if (currentStep === 8) {
      ...
      if (step8Phase === "done") return handleComma(s8.navTextDone);
      if (step8Phase === "idle") return handleComma(s8.navTextInitial);
      return "";
    }
    if (currentStep === 9) {
      const s9 = APP_DATA.steps[9];
      if (step9Part === "x") {
        return handleComma(
          step9XStatus === "correct" ? "" : s9.navTextInitial,
        );
      }
  ...
  const isNextDisabled =
    ...
    (currentStep === 9 && !(step9Part === "y" && step9YStatus === "correct"));
  ...
  useEffect(() => {
    const updateNudges = () => {
      ...
      if (currentStep === 0) {
        ...
        if (step9Part === "y" && step9YStatus === "correct") {
          addNudgeFor("next-button");
        }
