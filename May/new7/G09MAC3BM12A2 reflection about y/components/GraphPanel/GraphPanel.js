...
const GraphPanel = (props) => {
  ...
  const xArrowDims = getArrowDims("normal");
  const yArrowDims = getArrowDims(xAxisHighlighted ? "large" : "normal");
  const xArrowColor = AXIS_COLOR;
  const yArrowColor = xAxisHighlighted ? AXIS_HIGHLIGHT : AXIS_COLOR;
  ...
  const axisLabels = useMemo(() => {
    ...
    for (let i = X_MIN + 1; i < X_MAX; i++) {
      ...
      const isCoordinateHighlight =
        (step === 6 && step6HighlightX2 && i === -5) ||
        (step === 7 && step7Answer === "correct" && (i === 5 || i === -5));
      ...
      if (i === -5 || i === 5) {
        const boxWidth = String(i).length * labelFontSize * 0.58;
    ...
    for (let j = Y_MIN + 1; j < Y_MAX; j++) {
      ...
      if (j === 4) {
        const boxWidth = String(j).length * labelFontSize * 0.7;
        const boxHeight = labelFontSize * 1.05;
        els.push(
          React.createElement(
            "g",
            ...
            React.createElement("rect", {
              key: "yl-bg-" + j,
              x: ORIGIN_X - 18 - boxWidth / 2,
              y: py + 5 - labelFontSize * 0.82,
              width: boxWidth,
              height: boxHeight,
              fill: "#0a2b3f",
            }),
  ...
  const axisFoot = toSvg(0, TARGET_A.y);
  const aPrimePt = toSvg(-5, 4);
  const xAxisFootA = toSvg(TARGET_A.x, 0);
  const aPrimeAxisFoot = toSvg(-5, 0);
  const yAxisFootA = toSvg(0, TARGET_A.y);
  const yAxisFootAPrime = toSvg(0, 4);

  const dashedLen = Math.abs(targetPt.x - axisFoot.x);
  ...
  const renderPointLabel = () => {
    ...
    const labelX = pt.x - 50;
    const labelY = pt.y - 12;
    const showHighlightX = highlightFour && step === 4 && step4Phase === "done";
    const highlightStep7X = step === 7 && step7Answer === "correct";
    const highlightStep7Y = step === 7 && step7Answer === "wrong";
    ...
    return React.createElement(
      ...
      React.createElement("tspan", { opacity: highlightStep7X ? COORD_DIM_OPACITY : 1 }, "A("),
      React.createElement(
        "tspan",
        {
          fill:
            showHighlightX || highlightStep7X
              ? highlightStep7X
                ? COORD_ORANGE
                : YELLOW
              : highlightStep8X
                ...
                : WHITE,
          fontWeight:
            showHighlightX || highlightStep7X || highlightStep8X ? "700" : "600",
          opacity: 1,
          className: step8XBlink ? "rg-blink-text" : undefined,
        },
        String(pointCoords.x),
      ),
      React.createElement("tspan", { opacity: highlightStep7X ? COORD_DIM_OPACITY : 1 }, ", "),
      React.createElement(
        "tspan",
        {
          fill:
            highlightStep7Y
              ...
              : highlightStep8Y
                ...
                : WHITE,
          fontWeight:
            highlightStep7Y || highlightStep8Y ? "700" : "600",
  ...
  const renderUnitLabel = () => {
    ...
    const lineY = toSvg(0, TARGET_A.y).y;
    let labelX;

    if (isProp2Measuring) {
      const labelMathX = unitLabelOverride
        ? unitLabelOverride.x
        : (unitLineY1 + unitLineY2) / 2;
      const midX = toSvg(labelMathX, TARGET_A.y).x;
      return React.createElement(
        "text",
        {
          x: midX,
    ...
    if (unitLabelFinal) {
      labelX = (targetPt.x + axisFoot.x) / 2;
    } else {
      const midX = (unitLineY1 + unitLineY2) / 2;
      labelX = toSvg(midX, TARGET_A.y).x;
    }
  ...
  const renderQ1FourUnitsLabel = () => {
    ...
    const labelX = (targetPt.x + axisFoot.x) / 2;
    const labelY = toSvg(0, TARGET_A.y).y + 34;
    const q1Text = APP_DATA.steps[4].unitPlural.replace("{n}", "5");
  ...
  const renderQ4FourUnitsLabel = () => {
    ...
    const labelText = APP_DATA.steps[4].unitPlural.replace("{n}", "5");
    return React.createElement(
      "text",
      {
        x: toSvg(-2.5, 4).x,
  ...
  const renderAPrimeLabel = () => {
    const labelX = aPrimePt.x - 76;
    const labelY = aPrimePt.y - 12;
    const highlightX =
      (step === 7 && step7Answer === "correct") || (step === 8 && step8XActive);
    const highlightY =
      (step === 7 && step7Answer === "wrong") || (step === 8 && step8YActive);
    const dimNonXParts = step === 7 && step7Answer === "correct";
    ...
    return React.createElement(
      ...
      React.createElement("tspan", { opacity: dimNonXParts ? COORD_DIM_OPACITY : 1 }, "A\u2032("),
      React.createElement(
        "tspan",
        {
      ...
      React.createElement("tspan", { opacity: dimNonXParts ? COORD_DIM_OPACITY : 1 }, ", "),
      React.createElement(
        "tspan",
        {
          fill:
            step === 8 && step8YActive
              ...
              : highlightY
                ? COORD_ORANGE
                : WHITE,
          opacity: step === 6 && !step6ShowCoordY ? 0 : dimNonXParts ? COORD_DIM_OPACITY : 1,
          className: step8YBlink ? "rg-blink-text" : undefined,
        },
        "4",
      ),
  ...
  const renderStep6CoordinateClones = () => {
    const labelX = aPrimePt.x - 76;
    const labelY = aPrimePt.y - 12;
    const xSource = { x: toSvg(-5, 0).x, y: ORIGIN_Y + 24 };
  ...
  const renderStep7WrongClones = () => {
    ...
    const destination = { x: ORIGIN_X - 18, y: toSvg(0, 4).y + 5 };
    const sources = [
      { key: "a", x: targetPt.x + 12, y: targetPt.y - 12 },
      { key: "ap", x: aPrimePt.x - 18, y: aPrimePt.y - 12 },
    ];
  ...
  const getCalloutTransform = (pos) =>
    pos === "q1"
      ? "translate(" + (ORIGIN_X + 0.9 * UNIT) + "px," + (ORIGIN_Y - 2.8 * UNIT) + "px)"
      : "translate(" + (ORIGIN_X - 6.7 * UNIT) + "px," + (ORIGIN_Y - 2.8 * UNIT) + "px)";
  ...
  const unitLinePt1 = toSvg(unitLineY1, TARGET_A.y);
  const unitLinePt2 = toSvg(unitLineY2, TARGET_A.y);
  const reflectionLabelText = APP_DATA.steps[3].reflectionLabel || "";
  const reflectionLabelParts = reflectionLabelText.includes("y-axis")
    ? {
        before: reflectionLabelText.split("y-axis")[0],
        after: reflectionLabelText.split("y-axis").slice(1).join("y-axis"),
        prefix: "",
        suffix: "-axis",
      }
    : reflectionLabelText.includes("sumbu-y")
      ? {
          before: reflectionLabelText.split("sumbu-y")[0],
          after: reflectionLabelText.split("sumbu-y").slice(1).join("sumbu-y"),
          prefix: "sumbu-",
          suffix: "",
        }
      : null;

  const xAxisStroke = AXIS_COLOR;
  const xAxisWidth = 2;
  const xAxisOpacity = xAxisHighlighted ? AXIS_DIM_OPACITY : 1;
  const yAxisStroke = xAxisHighlighted ? AXIS_HIGHLIGHT : AXIS_COLOR;
  const yAxisWidth = xAxisHighlighted ? 4 : 2;
  ...
  const cloneBase = toSvg(5, 4);
  const cloneDest = toSvg(cloneY == null ? 0 : cloneY, 4);
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "g",
          { opacity: yAxisOpacity },
          renderAxisArrow(ORIGIN_X, plotTop, "up", yArrowColor, yArrowDims),
          renderAxisArrow(ORIGIN_X, plotBottom, "down", yArrowColor, yArrowDims),
        ),
        ...
        showProp1Overlays && p1LineVisible
          ? React.createElement("line", {
              ...
              x1: plotRight,
              y1: toSvg(0, TARGET_A.y).y,
              x2: plotLeft,
              y2: toSvg(0, TARGET_A.y).y,
              stroke: WHITE,
              strokeWidth: 2.5,
              strokeDasharray: "7 5",
              strokeLinecap: "round",
        ...
        step === 5 && showMeasureLine && measureLineUnits > 0
          ? React.createElement("line", {
              ...
              x1: toSvg(0, TARGET_A.y).x,
              y1: toSvg(0, TARGET_A.y).y,
              x2: toSvg(-measureLineUnits, TARGET_A.y).x,
              y2: toSvg(-measureLineUnits, TARGET_A.y).y,
              stroke: YELLOW,
              strokeWidth: 4,
              strokeDasharray: "7 5",
            })
