const Button = ({ onClick, text, className, id }) => {
  return React.createElement(
    "button",
    {
      className: `btn ${className || ""}`,
      onClick: onClick,
      id: id,
    },
    text
  );
};
const Fullscreen = ({ text, subText, buttonText, onButtonClick, heading, left = false, isFinal = false, imageSrc, buttonId }) => {
  const hasTwoColumns = isFinal && imageSrc;
  const contentArea = hasTwoColumns
    ? React.createElement(
        "div",
        { className: "fullscreen-two-column" },
        React.createElement("div", { className: "fullscreen-image-col" },
          React.createElement("img", { src: imageSrc, alt: "", className: "fullscreen-final-image" })
        ),
        React.createElement("div", {
          className: "fullscreen-text-col fullscreen-content center",
          dangerouslySetInnerHTML: { __html: text },
        })
      )
    : React.createElement(
        "div",
        { className: "fullscreen-copy-group" },
        React.createElement("p", {
          className: "fullscreen-content " + (left ? "left" : "center"),
          dangerouslySetInnerHTML: { __html: text },
        }),
        subText
          ? React.createElement("p", {
              className: "fullscreen-sub-content " + (left ? "left" : "center"),
              dangerouslySetInnerHTML: { __html: subText },
            })
          : null,
      );
  return React.createElement(
    "div",
    { className: "fullscreen-panel" },
    heading
      ? React.createElement("p", { className: "heading", dangerouslySetInnerHTML: { __html: heading } })
      : null,
    contentArea,
    React.createElement(Button, {
      text: buttonText,
      onClick: onButtonClick,
      className: "fullscreen-button",
      id: buttonId,
    })
  );
};
