const NUDGE_DELAY_MS = 1000;
const PREVIOUS_STEP_CHECKPOINTS = [1, 2, 3, 5, 6];

function getPreviousCheckpoint(step) {
  for (let i = PREVIOUS_STEP_CHECKPOINTS.length - 1; i >= 0; i -= 1) {
    if (PREVIOUS_STEP_CHECKPOINTS[i] < step) {
      return PREVIOUS_STEP_CHECKPOINTS[i];
    }
  }
  return 1;
}

const App = () => {
  const { useState, useEffect, useCallback, useRef } = React;

  const [currentStep, setCurrentStep] = useState(1);
  const [isNextDisabled, setIsNextDisabled] = useState(false);
  const [dynamicNavText, setDynamicNavText] = useState(null);
  const [dynamicQuestionText, setDynamicQuestionText] = useState(null);
  const [nextButtonText, setNextButtonText] = useState("\u00BB");
  const [resetKey, setResetKey] = useState(0);
  const [showNudge, setShowNudge] = useState(false);
  const [nudgePosition, setNudgePosition] = useState(null);
  const [isNavLocked, setIsNavLocked] = useState(false);
  const [stepTransition, setStepTransition] = useState(null);
  const nudgeElementRef = useRef(null);
  const nudgeDelayRef = useRef(null);

  const hideNudge = useCallback(() => {
    if (nudgeDelayRef.current) {
      clearTimeout(nudgeDelayRef.current);
      nudgeDelayRef.current = null;
    }
    nudgeElementRef.current = null;
    setShowNudge(false);
    setNudgePosition(null);
  }, []);

  const handleRestart = useCallback(() => {
    hideNudge();
    setCurrentStep(1);
    setIsNextDisabled(false);
    setDynamicNavText(null);
    setDynamicQuestionText(null);
    setNextButtonText("\u00BB");
    setIsNavLocked(false);
    setStepTransition(null);
    setResetKey((k) => k + 1);
  }, [hideNudge]);

  const handleNext = useCallback(
    (overrideStep, silent) => {
      if (!silent && typeof playSound === "function") playSound("click");
      hideNudge();
      if (typeof overrideStep === "number") {
        setStepTransition(null);
        setCurrentStep(overrideStep);
        return;
      }
      if (currentStep === 1 && !isNextDisabled) {
        setStepTransition(null);
        setCurrentStep(2);
        return;
      }
      if (currentStep === 2 && !isNextDisabled) {
        setStepTransition(null);
        setCurrentStep(3);
        return;
      }
      if (currentStep === 3 && !isNextDisabled) {
        setStepTransition("3to4");
        setCurrentStep(4);
        return;
      }
      if (currentStep === 4 && !isNextDisabled) {
        setStepTransition(null);
        setCurrentStep(5);
        return;
      }
      if (currentStep === 5 && !isNextDisabled) {
        setStepTransition(null);
        setCurrentStep(6);
        return;
      }
      if (currentStep === 6 && !isNextDisabled) {
        setStepTransition("6to7");
        setCurrentStep(7);
        return;
      }
      if (currentStep === 7 && !isNextDisabled) {
        setStepTransition(null);
        setCurrentStep(8);
        return;
      }
      if (currentStep === 8 && !isNextDisabled) {
        handleRestart();
      }
    },
    [currentStep, handleRestart, hideNudge, isNextDisabled],
  );

  const setNextEnabled = useCallback((enabled) => {
    setIsNextDisabled(!enabled);
  }, []);

  const handlePrev = useCallback(() => {
    if (typeof playSound === "function") playSound("click");
    hideNudge();
    setStepTransition(null);
    setIsNavLocked(false);
    setCurrentStep((prev) => getPreviousCheckpoint(prev));
    setResetKey((k) => k + 1);
  }, [hideNudge]);

  const updateTexts = useCallback((question, nav) => {
    if (question !== undefined) setDynamicQuestionText(question);
    if (nav !== undefined) setDynamicNavText(nav);
  }, []);

  const setNextLabel = useCallback((text) => {
    if (text !== undefined) setNextButtonText(text);
  }, []);

  const registerNudgeTarget = useCallback((target, options) => {
    const opts = options || {};
    const delay = opts.immediate
      ? 0
      : opts.delay !== undefined
        ? opts.delay
        : NUDGE_DELAY_MS;

    if (nudgeDelayRef.current) {
      clearTimeout(nudgeDelayRef.current);
    }
    nudgeElementRef.current = target;
    setShowNudge(false);
    setNudgePosition(null);

    nudgeDelayRef.current = setTimeout(() => {
      const el = nudgeElementRef.current;
      const rect =
        el && typeof el.getBoundingClientRect === "function"
          ? el.getBoundingClientRect()
          : el;
      if (!rect) return;
      setNudgePosition(rect);
      setShowNudge(true);
      nudgeDelayRef.current = null;
    }, delay);
  }, []);

  useEffect(() => {
    if (currentStep === 1) {
      setDynamicNavText(null);
      setDynamicQuestionText(null);
      setIsNextDisabled(false);
      setNextButtonText("\u00BB");
    }
    if (currentStep === 2) {
      setDynamicNavText(null);
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
    }
    if (currentStep === 3) {
      setDynamicNavText(null);
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
    }
    if (currentStep === 4) {
      setDynamicNavText("");
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
    }
    if (currentStep === 5) {
      setDynamicNavText("");
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
      setNextButtonText("\u00BB");
    }
    if (currentStep === 6) {
      setDynamicNavText(null);
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
      setNextButtonText("\u00BB");
    }
    if (currentStep === 7) {
      setDynamicNavText(null);
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
      setNextButtonText("\u00BB");
    }
    if (currentStep === 8) {
      setDynamicNavText("");
      setDynamicQuestionText(null);
      setIsNextDisabled(true);
      setNextButtonText("\u00BB");
    }
  }, [currentStep]);

  const getQuestionText = () => {
    if (dynamicQuestionText !== null) return dynamicQuestionText;
    const stepData = APP_DATA.steps[currentStep];
    return stepData ? stepData.questionText : "";
  };

  const getNavText = () => {
    if (dynamicNavText !== null && dynamicNavText !== undefined) {
      return dynamicNavText;
    }
    const stepData = APP_DATA.steps[currentStep];
    return stepData ? stepData.navText : "";
  };

  useEffect(() => {
    const updateNudge = () => {
      if (currentStep === 1 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 2 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 3 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 3 && isNextDisabled) {
        if (showNudge && nudgeElementRef.current) {
          const rect = nudgeElementRef.current.getBoundingClientRect();
          setNudgePosition(rect);
        }
        return;
      }
      if (currentStep === 4 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 4 && isNextDisabled) {
        hideNudge();
        return;
      }
      if (currentStep === 5 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 5 && isNextDisabled) {
        hideNudge();
        return;
      }
      if (currentStep === 6 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 6 && isNextDisabled) {
        if (showNudge && nudgeElementRef.current) {
          const rect = nudgeElementRef.current.getBoundingClientRect();
          setNudgePosition(rect);
        }
        return;
      }
      if (currentStep === 7 && !isNextDisabled) {
        const nextBtn = document.getElementById("next-button");
        if (nextBtn) {
          setNudgePosition(nextBtn.getBoundingClientRect());
          setShowNudge(true);
        }
        return;
      }
      if (currentStep === 7 && isNextDisabled) {
        if (showNudge && nudgeElementRef.current) {
          const rect = nudgeElementRef.current.getBoundingClientRect();
          setNudgePosition(rect);
        }
        return;
      }
      if (currentStep === 8) {
        hideNudge();
        return;
      }
      hideNudge();
    };

    const timeoutId = setTimeout(updateNudge, 200);
    window.addEventListener("resize", updateNudge);
    return () => {
      clearTimeout(timeoutId);
      window.removeEventListener("resize", updateNudge);
    };
  }, [
    currentStep,
    isNextDisabled,
    dynamicNavText,
    hideNudge,
    resetKey,
    showNudge,
  ]);

  return React.createElement(
    "div",
    { className: "applet-container" },
    React.createElement(QuestionPanel, {
      text: getQuestionText(),
      step: currentStep,
    }),
    React.createElement(
      "div",
      { className: "app-main-content" },
      React.createElement(MainCanvas, {
        key: resetKey,
        step: currentStep,
        onSetNextEnabled: setNextEnabled,
        onUpdateTexts: updateTexts,
        onSetNextLabel: setNextLabel,
        onNext: handleNext,
        onRegisterNudgeTarget: registerNudgeTarget,
        onHideNudge: hideNudge,
        onSetNavLocked: setIsNavLocked,
        stepTransition: stepTransition,
      }),
    ),
    React.createElement(
      "div",
      { className: "lower-panel" },
      React.createElement(Navigation, {
        onNav: (dir) => (dir === "next" ? handleNext() : handlePrev()),
        isNextDisabled: isNextDisabled,
        isPrevDisabled: currentStep === 1 || isNavLocked,
        navText: getNavText(),
        nextButtonText: nextButtonText,
        step: currentStep,
      }),
    ),
    React.createElement(Nudge, { show: showNudge, position: nudgePosition }),
  );
};
