const App = () => {
  ...
  const runGeneralRuleAnimation = useCallback((startVal, endVal, axis, onComplete) => {
    ...
    if (typeof stopAutoSliderSound === "function") stopAutoSliderSound();

    if (typeof startAutoSliderSound === "function") startAutoSliderSound();
  ...
  const runStep6LineAnim = useCallback((hVal, vVal) => {
    ...
    const showVertical = () => {
      if (Math.abs(dy) > 0.05) {
        if (typeof playSound === "function") playSound("zoom");
        setStep6LinePhase("v");
