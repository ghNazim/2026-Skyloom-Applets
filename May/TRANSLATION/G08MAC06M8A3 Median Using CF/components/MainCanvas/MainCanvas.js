const MainCanvas = (props) => {
  ...
  // The Cumulative Frequency column only appears AFTER the table has slid to its CF
  // position — not the instant step 3 opens. Restored/direct entry shows it right away.
  const [cfColumnVisible, setCfColumnVisible] = useState(step >= 3);
  ...
  const [medianBlink, setMedianBlink] = useState(false);   // pulse the median-position cell on a repeat wrong
  const rangeShownRef = useRef(false);                      // has the range column already been revealed?
  ...
  function handleFormulaChoice(option) {
    ...
    play("correct");
    setSelectedFormulaId(option.id);          // wrong option collapses, correct snaps up
    setStep4FeedbackHtml("");
    setTimeout(function () {
      setFormulaTransferDone(true);           // box settles to its resolved (answer) state
    }, 450);
    // Hold the "answer under Median position" beat, then flow into the calculation.
    setTimeout(function () {
  ...
  // Replay a yellow blink on the median-position cell. Double rAF so the class is
  // removed then re-added across frames, which restarts the CSS animation.
  function pulseMedianCell() {
    setMedianBlink(false);
    window.requestAnimationFrame(function () {
      window.requestAnimationFrame(function () { setMedianBlink(true); });
    });
  }
  ...
  function handleMedianNumpadSubmit() {
    ...
    if (value !== 15) {
      ...
      // First wrong answer: reveal the position-range column as a hint. On any FURTHER
      // wrong answer, don't re-reveal it (that re-runs the whole column animation) —
      // instead blink the highlighted median cell to point at where the median lies.
      if (!rangeShownRef.current) {
        rangeShownRef.current = true;
  ...
  useEffect(function () {
    ...
    if (step === 3) {
      ...
      // The table finishes sliding to its CF position (~0.6s), THEN the CF column
      // appears, and the numpad follows.
      setTimeout(function () {
        setCfColumnVisible(true);
      }, 700);
      setTimeout(function () {
        setCfActionVisible(true);
      }, 720);
  ...
  function renderDropdown() {
    ...
    if (dropdown.type === "data") {
      ...
      if (activeDataCell && tableWrap) {
        ...
        // Hug the RIGHT side of the data cell (slight overlap in), the same way the
        // frequency dropdown sits on the right side of its cell — not floated off in the
        // gap. A small negative offset keeps it attached to the cell rather than "away".
        dropdownStyle.left = (cellRect.right - wrapRect.left - cellRect.width * 0.05) + "px";
          ...
          return e("div", { className: "median-table-row", key: "row-" + index },
            ...
            showCf ? e("div", {
              className: "median-table-cell cf-cell" +
                ...
                // Once the CF column is complete and we move to the "n" question (step 4),
                // the pink cells recede a little so focus shifts to the n entry box. This
                // recede must PERSIST through step 5 (finding the median) to the end — the
                // column stays a supporting reference, so it should never snap back to full
                // pink once the formula is chosen. The stronger total-hint dim (during the
                // hint) takes over from this within step 4.
                (step >= 4 && cfFilled && !totalHintDim && !totalHintLast ? " cf-focus-dim" : ""),
            ...
            showRange ? e("div", {
              className: "median-table-cell range-cell" +
                ...
                (medianRangeHint && medianBlink ? " median-blink" : ""),
  ...
  function renderOrdinalFormula(children, leadingNilai) {
    if (current_language === "id") {
      return e("span", { className: "formula-core" },
        e("span", { className: "formula-prefix" }, leadingNilai ? "Nilai ke-" : "ke-"),
        children
      );
    }
      ...
      APP_DATA.steps[4].options.map(function (option) {
        return e("button", {
          ...
          className: "formula-option" +
            ...
            (selectedFormulaId === option.id ? " chosen" : "") +
            (selectedFormulaId && selectedFormulaId !== option.id ? " option-gone" : ""),
  ...
  function renderStep4ActionColumn() {
    var formulaMode = step4Mode === "formula";
    return e("div", { className: "step4-action-column" + (formulaMode ? " step4-formula-mode" : ""), ref: step4ActionRef },
      ...
      // Formula question: the "Median position" box holds the label + the two options
      // directly (no empty placeholder slot). On the correct tap the WRONG option
      // collapses away and the correct one snaps up under the label — the box shrinks to
      // just the answer, then the calculation flows on. The wrong-answer hint sits BELOW
      // the box, inside the panel.
      formulaMode ? e("div", { className: "step4-median-block" + (formulaTransferDone ? " resolved" : ""), ref: step4MedianPositionRef },
        e("div", { className: "middle-position-label step5-position-label" }, APP_DATA.steps[5].medianPosition),
        e("div", { className: "step4-options-wrap" },
          renderFormulaOptions()
        )
      ) : null,
      e("div", { className: "step4-feedback-row" },
        (step4FeedbackHtml && !(formulaMode && selectedFormulaId)) ? e("div", {
          className: "feedback-card step4-feedback-card",
          dangerouslySetInnerHTML: { __html: step4FeedbackHtml },
        }) : null
      ),
      step4Mode === "count" ? e("div", { className: "step4-control-row" }, renderFlatNumpad()) : null
    );
  }
        ...
        e("div", { className: "median-equation" },
          ...
          e("div", {
            className: "median-answer-box" +
              ...
              (!medianError && !medianCorrect ? " active" : ""),
  ...
  return e("div", { className: "main-canvas-container" + (isCfStage ? " cf-stage" : "") },
    e("div", { className: "visual-row" },
      ...
      e("div", { className: "right-visual-column" + (showRight && rightRevealed ? " visible" : "") + (cfLayoutActive ? " cf-table-column" : "") },
        showRight ? renderFrequencyTable(cfColumnVisible) : null
      ),
      isCfStage ? e("div", {
