...
function setAllCalcPartOpacity(parts, opacity) {
  const next = {};
  Object.keys(parts).forEach((key) => {
    next[key] = { ...parts[key], opacity };
  });
  return next;
}
...
function makeS6RatioValueSlots() {
  ...
  return {
    ...
    rdh: { x: rhs, y: y + 18, text: heightVar, c: "white", o: 0, italic: true },
  };
}
...
function makeS6Line2() {
  ...
  const fx = cx - 18;
  return {
    h: { x: cx - 78, y, text: heightVar, c: "white", o: 0, italic: true },
...
function makeS6Line4(multOnly) {
  ...
  if (multOnly) {
    return {
      h: { x: cx - 54, y, text: heightVar, c: "white", o: 0, italic: true },
  ...
  return {
    h: { x: cx - 54, y, text: heightVar, c: "white", o: 1, italic: true },
    eq: { x: cx - 30, y, text: "=", c: "white", o: 1 },
    val: { x: cx + 16, y, text: APP_DATA.labels.heightTreeResult, c: "white", o: 0 },
  };
}
...
const MainCanvas = (props) => {
  ...
  useEffect(() => {
    ...
    setStep2Done(true);
    setCalcPanelVisible(true);
    setCalcTitleOpacity(1);
    setLine1Parts(setAllCalcPartOpacity(getCalcLine1Positions(), 1));
    setShowRightSquares(true);
    setRightSquaresStable(false);
    setSquareBlink(1);
    setDiagramShiftX(DIAGRAM_SHIFT_LEFT);
    setYellowOffsetY(YELLOW_FLOAT_OFFSET);
    setFloatingLabels(true);
  ...
  useEffect(() => {
    ...
    setLine1Parts(setAllCalcPartOpacity(getCalcLine1Positions(), 1));
    setLine2Visible(true);
    setLine2Parts(setAllCalcPartOpacity(getCalcLine2Positions(), 1));
    setDiagramShiftX(DIAGRAM_SHIFT_LEFT);
    setYellowOffsetY(0);
    setFloatingLabels(false);
    setShowRightSquares(true);
  ...
  const runS6Line2Anim = useCallback(async () => {
    ...
    await flyFromRatio("rdh", "h", heightVar);
