const GameRow = (props) => {
  ...
  const renderEventCell = (row, col, rowIndex, colIndex) => {
    ...
    if (headerOn) className += " pending-reveal";
  ...
  colItems.forEach((col, colIndex) => {
    const glow =
      headerHighlight && headerHighlight.col === col ? " header-glow" : "";
    gridChildren.push(
      e(
        "div",
        {
  ...
  rowItems.forEach((row, rowIndex) => {
    const glow =
      headerHighlight && String(headerHighlight.row) === String(row)
        ? " header-glow"
        : "";
    gridChildren.push(
      e(
  ...
  return e(
    ...
    showLives
      ...
      : null,
    e(Nudge, {
      targetRef: highlightedCellRef,
      active: showNudge && !!highlightedKey && !cellState[highlightedKey],
