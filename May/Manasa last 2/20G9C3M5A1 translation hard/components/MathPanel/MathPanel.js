const MathPanel = ({
  ...
  const hlSegment = (forStep, oldContent, newContent) => {
    ...
    if (anim === "fade-out" && step === forStep) {
      return renderHighlight(oldContent, false, "fade-out");
    }
    if (anim === "fade-in" && step === forStep + 1) {
      return renderHighlight(newContent, false, "fade-in");
    }
  ...
  const renderFormula = () => {
    ...
    if (step === 4 || (anim === "fade-in" && step === 5)) {
      return hlSegment(4, rowY3EqNegX2(), rowXYEq32());
    }

    if (step === 3 || (anim === "fade-in" && step === 4)) {
      if (anim === "fade-in" && step === 4) {
        return hlSegment(3, distGroup(), rowY3EqNegX2());
      }
      return React.createElement(
        "span",
        { className: "math-formula-eq-inner" },
    ...
    if (step === 2 || (anim === "fade-in" && step === 3)) {
      return React.createElement(
        ...
        anim === "fade-in" && step === 3 ? null : rhsParenX2(),
      );
    }

    if (step === 1 || (anim === "fade-in" && step === 2)) {
      return React.createElement(
        "span",
        { className: "math-formula-eq-inner" },
        lhsYMinus3(),
