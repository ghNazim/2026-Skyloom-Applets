...
const RectMainCanvas = ({
    ...
    return new Promise((resolve) => {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          if (typeof playSoundDelayed === "function") {
            playSoundDelayed("swoosh", 200);
          }
        });
      });
  ...
  const runFindAnimation = useCallback(
    async (primeKey) => {
      ...
      await animateFly(ruleRefs.current.imgNegYNum, ruleRefs.current.objY, {
        text: objCoord.y,
      ...
      await animateFly(ruleRefs.current.imgNegXNum, ruleRefs.current.objX, {
        text: objCoord.x,
        colorClass: "is-orange",
      });
      if (isCancelled()) return;
  ...
  const runRotationAnimation = useCallback(async () => {
    ...
    setPointStates({
      APrime: { circleOpacity: 0.22, labelVisible: true, labelOpacity: 0.22 },
      BPrime: { circleOpacity: 0.22, labelVisible: true, labelOpacity: 0.22 },
      CPrime: { circleOpacity: 0.22, labelVisible: true, labelOpacity: 0.22 },
    ...
    setRotationOverlay({
      ...
      points: clonePoints,
      segments: [
        ...RECT_OBJECT_VERTICES.map((vertex) => ({
          from: { x: 0, y: 0 },
          to: vertex,
          color: "#ffffff",
          opacity: 0.6,
    ...
    setPointStates({
      APrime: { circleOpacity: 1, labelVisible: true },
      BPrime: { circleOpacity: 1, labelVisible: true },
      CPrime: { circleOpacity: 1, labelVisible: true },
  ...
  useEffect(() => {
    ...
    const initFindStep = (findStep, phaseKey) => {
      ...
      setRightVisible(true);
      findAnimRef.current = false;
      setObjectPointStates(restoreObjectPointsForStep(findStep));
      setShowObjectRect(false);
      setObjectRectOpacity(0);
    };
