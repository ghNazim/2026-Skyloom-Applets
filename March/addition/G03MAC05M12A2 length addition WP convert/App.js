/** Gram (cm) box display: pad to 2 digits while typing; up to 3 chars max. */
...
const App = () => {
  ...
  /** Keep step 3 rows done; clear MCQ + steps 5–6 (used when going 5 → 4). */
  const clearProgressFromStep4Onward = () => {
    setMcqSelected(null);
    setMcqCorrect(null);
    setStep4Done(false);
    setShowHintHighlight(false);
  ...
  /** Keep steps 3–4 done; clear result row + step 5–6 progress (used when going 6 → 5). */
  const clearProgressFromStep5Onward = () => {
    setResultGm("");
    setResultKg("");
    setResultGmStatus("");
    setResultKgStatus("");
  ...
  const getActiveMaxLen = () => {
    if (currentStep === 3) return activeBox.includes("kg") ? 2 : 3;
  ...
  const isActiveGramBox = () =>
    currentStep === 5 ||
    (currentStep === 3 && (activeBox === "r1gm" || activeBox === "r2gm"));
  ...
  const handleStep5Submit = () => {
    if (resultGm === "") return;
    const gmExpectedStep5 =
      typeof q.gmInterim === "number" ? q.gmInterim : q.gmResult;
    const gmVal = parseInt(resultGm, 10);
    if (gmVal === gmExpectedStep5) {
      setResultGmStatus("correct");
      doCorrectFeedback(q.step5CorrectFeedback);
      setResultGm(String(gmExpectedStep5));
      setStep5GmDone(true);
      setNumpadDisabled(true);
    } else {
  ...
  const handleStep6Submit = () => {
    ...
    if (kgVal === q.kgResult) {
      ...
      if (typeof q.gmInterim === "number") setResultGm(String(q.gmResult));
      setStep6Done(true);
      setNumpadDisabled(true);
    } else {
  ...
  const handleMcqClick = (option) => {
    ...
    const correctOp =
      q.operation === "add"
        ? APP_DATA.mcqOptions.add
        : APP_DATA.mcqOptions.subtract;
  ...
  const handlePrev = () => {
    ...
    if (currentStep === 5) {
      clearProgressFromStep4Onward();
      setCurrentStep(4);
      return;
    }

    if (currentStep === 6) {
      clearProgressFromStep5Onward();
      setCurrentStep(5);
    }
  };
  ...
  if (currentStep === 3) {
    ...
    navText =
      q.operation === "add"
        ? APP_DATA.common.tapNextToAddWeights
        : APP_DATA.common.tapNextToSubtract;
    ...
    if (step5GmDone)
      navText =
        q.operation === "add"
          ? APP_DATA.common.tapNextToAddKg
          : APP_DATA.common.tapNextToSubtractKg;
  } else if (currentStep === 6) {
    navText = APP_DATA.common.tapNumpadToFill;
    nextDisabled = !step6Done;
    if (step6Done)
      navText = isLastQuestion
        ? APP_DATA.common.tapNextToSummarise
        : APP_DATA.common.tapNextForAnother;
  }
  ...
  if (currentStep === 1) {
    ...
    characterText = q.step3CharacterText || APP_DATA.steps[3].characterText;
  else if (currentStep === 4) characterText = APP_DATA.steps[4].characterText;
  else if (currentStep === 5)
    characterText = step5GmDone ? q.step5CtAfterGm : q.step5CharacterText;
  else if (currentStep === 6)
    characterText = step6Done
      ? isLastQuestion
        ? q.step6FinalCtLast
        : q.step6FinalCt
      : q.step5CtAfterGm;
  ...
  const renderCalcRow = (opts) => {
    ...
    return e(
      ...
      e(
        "div",
        {
          className: "calc-cell " + (showLabel ? "item-name" : "operator-sign"),
        },
        showLabel ? label : operator || "",
      ),
  ...
  if (showQuestion) {
    leftChildren.push(
      e("div", {
        ...
        dangerouslySetInnerHTML: { __html: getStatementHtml() },
      }),
    );
  }
  ...
  if (showCalcTable) {
    ...
    rows.push(
      e(
        ...
        renderCalcRow({
          label: q.itemName1,
          showLabel: !inCalcMode,
          operator: null,
          kgVal: getRowVal(row1Kg, q.kgAns1, row1KgStatus),
          gmVal: getRowVal(row1Gm, q.gmAns1, row1GmStatus),
          kgStatus: inCalcMode ? "" : row1KgStatus,
          gmStatus: inCalcMode ? "" : row1GmStatus,
          kgActive: currentStep === 3 && activeBox === "r1kg",
          gmActive: currentStep === 3 && activeBox === "r1gm",
    ...
    rows.push(
      e(
        ...
        renderCalcRow({
          label: q.itemName2,
          showLabel: !inCalcMode,
          operator: inCalcMode ? opSymbol : null,
          kgVal: getRowVal(row2Kg, q.kgAns2, row2KgStatus),
          gmVal: getRowVal(row2Gm, q.gmAns2, row2GmStatus),
          kgStatus: inCalcMode ? "" : row2KgStatus,
          gmStatus: inCalcMode ? "" : row2GmStatus,
          kgActive: currentStep === 3 && activeBox === "r2kg",
          gmActive: currentStep === 3 && activeBox === "r2gm",
      ...
      rows.push(
        e(
          ...
          renderCalcRow({
            ...
            kgVal: resultKg,
            gmVal: resultGm,
            kgStatus: resultKgStatus,
            gmStatus: resultGmStatus,
            kgActive: rKgActive,
            gmActive: rGmActive,
            kgClick: undefined,
            gmClick: undefined,
    ...
    leftChildren.push(
      e("div", { key: "ct", className: "calc-table" }, ...rows),
    );
  }
  ...
  if (showOperatorQ) {
    leftChildren.push(
      e("div", {
        key: "oq",
        className: "operator-question",
        dangerouslySetInnerHTML: { __html: APP_DATA.operatorQuestion },
      }),
    );
  }
  ...
  if (showNumpad) {
    ...
    rightContent = e(
      ...
      e(MCQPanel, {
        mcqData: {
          options: [APP_DATA.mcqOptions.add, APP_DATA.mcqOptions.subtract],
        },
  ...
  const leftCls =
    "content-left-column" + (leftChildren.length === 0 ? " empty-col" : "");
