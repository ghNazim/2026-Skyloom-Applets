const Comprehend = ({ step, substep, comprehendTileMagnifyClicked = false }) => {
  ...
  if (step === 1) {
    ...
    // During tile-magnify substep, keep given item-3 hidden until tile is tapped.
    const rawShowGivenCount = Math.min(substep + 1, totalGiven);
    const showGivenCount = (substep === 2 && !comprehendTileMagnifyClicked)
      ? Math.min(2, rawShowGivenCount)
      : rawShowGivenCount;
