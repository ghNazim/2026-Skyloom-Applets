const DragDropPanel = ({ onComplete, onEnableNext, onUpdateNavText, imageSrc, dragDropKey, step }) => {
  ...
  useEffect(() => {
    // Exclude static zones from completion check
    const nonStaticZones = dragDropData.dropZones.filter(zone => !zone.isStatic);
    const allFilled = nonStaticZones.every(zone => filledZones[zone.id]);
  ...
  const processDrop = (draggableId, zoneId) => {
    ...
    // Don't allow dropping on static zones
    if (zone.isStatic) return;

    let isCorrect = false;

    const mappedCorrectText = dragDropData.zoneMap && dragDropData.zoneMap[zoneId];

    if (mappedCorrectText) {
      isCorrect = draggable.text === mappedCorrectText;
    } else if (zone.correctAnswers) {
      // For interchangeable answers (like Length and Breadth), check if the text matches
      // and if it hasn't been used in another interchangeable zone
      const isInterchangeable = zone.correctAnswers.length > 1;
      if (zone.correctAnswers.includes(draggable.text)) {
        if (isInterchangeable) {
          // Check if this text is already used in another interchangeable zone
          const otherInterchangeableZones = dragDropData.dropZones.filter(
            z => z.correctAnswers && z.correctAnswers.length > 1 && z.id !== zoneId
          );
          const usedInOtherZone = otherInterchangeableZones.some(
            z => filledZones[z.id] && getDraggableText(filledZones[z.id]) === draggable.text
          );
          if (!usedInOtherZone) {
            isCorrect = true;
          }
        } else {
          if (!usedTextValues.includes(draggable.text)) {
            isCorrect = true;
          }
        }
      }
    ...
    if (isCorrect) {
      ...
      if (zone.correctAnswers && zone.correctAnswers.length === 1) {
  ...
  const renderDropZone = (zone, index) => {
    // Static zones are just text, not drop zones
    if (zone.isStatic) {
      return React.createElement(
        "span",
        {
          key: zone.id,
          className: "dd-static-text"
        },
        zone.placeholder || zone.correctAnswer
      );
    }
    ...
    if (dragDropData.showPlaceholder && !isFilled) className += " dd-placeholder-visible";
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        { className: "dd-image-row" },
          ...
          alt: APP_DATA.calculation.altTexts.riceField || APP_DATA.calculation.altTexts.milkBottles || "Rice field",
          className: "dd-image"
        })
      ),
      React.createElement(
        ...
        React.createElement(
          ...
          dragDropData.dropZones.map((zone, index) => {
            const element = renderDropZone(zone, index);
            // Add space after each element except the last
            return index < dragDropData.dropZones.length - 1
              ? React.createElement(React.Fragment, { key: zone.id }, element, " ")
              : React.createElement(React.Fragment, { key: zone.id }, element);
          })
        )
      )
