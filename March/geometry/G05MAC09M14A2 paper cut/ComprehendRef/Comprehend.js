const Comprehend = ({ step, substep, onShowFloorClick, showFloorButtonClicked }) => {
  ...
  if (step === 1) {
    ...
    const isSubstep0 = substep === 0;
    const showFloorLabel = comprehendData.showFloorButtonLabel || "Show Floor";
    
    return React.createElement(
      "div",
      { className: "comprehend-panel comprehend-panel-with-button" },
      // Section Title
      React.createElement(
        "h3",
        { className: "comprehend-section-title" },
      ...
      // Substep 0: "Show Floor" button (hide after tap)
      isSubstep0 && onShowFloorClick && !showFloorButtonClicked && React.createElement(
        "button",
        {
          type: "button",
          className: "comprehend-show-floor-btn",
          onClick: () => {
