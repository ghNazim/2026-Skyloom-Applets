const DragDropPanel = ({ appData, onComplete, onEnableNext, onUpdateNavText, imageSrc, dragDropKey, step, findingInstance }) => {
  ...
  // Reset state when dragDropKey or appData changes
  useEffect(() => {
    setFilledZones({});
  ...
  // Render drop zone (placeholder visible when empty until correct drop)
  ...
  // Build equation line: either equationLineSegments + dropZones or legacy 3-zone layout
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      // Findings div on top (from findingInstance or dragDropData)
      React.createElement(
        "div",
