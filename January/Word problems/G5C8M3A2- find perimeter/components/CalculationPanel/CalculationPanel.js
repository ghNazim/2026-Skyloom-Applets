const CalculationPanel = ({
  ...
  const isMultiBoxQuestion = Array.isArray(perimeterCalc.numpad1Answers);
  const isMultiBoxStep5 = step === 5 && isMultiBoxQuestion;
  const numpad1Answers = perimeterCalc.numpad1Answers;
  const numpad1Answer = perimeterCalc.numpad1Answer;
  ...
  const handleSubmit = () => {
    ...
    if (isStep5 && isMultiBoxStep5) {
      const currentBoxIndex = calcState.numpad1CurrentBoxIndex ?? 0;
      const correctAnswer = numpad1Answers[currentBoxIndex];
      if (numpadValue === correctAnswer) {
        ...
        setTimeout(() => {
          setInputCorrect(false);
          setNumpadValue("");
          const newBoxValues = [...(calcState.numpad1BoxValues || [])];
          newBoxValues[currentBoxIndex] = numpadValue;
          const nextIndex = currentBoxIndex + 1;
          if (nextIndex >= numpad1Answers.length) {
            setCalcState(prev => ({
              ...prev,
              numpad1Answered: true,
              numpad1BoxValues: newBoxValues,
            ...
            setCalcState(prev => ({
              ...prev,
              numpad1BoxValues: newBoxValues,
              numpad1CurrentBoxIndex: nextIndex
            }));
          }
        }, 300);
      } else {
    ...
    const correctAnswer = isStep5 ? numpad1Answer : perimeterCalc.numpad2Answer;
    if (numpadValue === correctAnswer) {
      setInputCorrect(true);
      if (window.playSound) window.playSound("correct");
      setTimeout(() => {
  ...
  const renderCalcRows = () => {
    ...
    if (step === 5) {
      if (!calcState.numpad1Answered) {
        if (isMultiBox) {
          ...
          rows.push(
            React.createElement("div", { key: "numpad1", className: "calc-row" },
              ...
              }, currentBoxIdx === 0 ? (numpadValue || "") : (boxValues[0] || "")),
              " + ",
              React.createElement("span", {
                className: `calc-input-box ${currentBoxIdx === 1 ? (inputError ? "error shake" : inputCorrect ? "correct" : "highlight") : box1Filled ? "correct" : ""}`
              }, currentBoxIdx === 1 ? (numpadValue || "") : (boxValues[1] || ""))
            )
          );
        } else {
          rows.push(
        ...
        if (isMultiBox) {
          const v0 = boxValues[0] || numpad1Answers[0];
    ...
    if (step === 7) {
      if (isMultiBox) {
        const v0 = calcState.numpad1BoxValues?.[0] || numpad1Answers[0];
        const v1 = calcState.numpad1BoxValues?.[1] || numpad1Answers[1];
        rows.push(
          React.createElement("div", { key: "val1", className: "calc-row" },
            "= 2 × " + v0 + " + " + v1
          )
