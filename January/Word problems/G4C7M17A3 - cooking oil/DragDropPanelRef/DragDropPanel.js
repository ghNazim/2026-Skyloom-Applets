const DragDropPanel = ({ onComplete, onEnableNext, onUpdateNavText, dragDropKey, step, showQuestionInLeft }) => {
  ...
  // 🔹 RESTORED VERTICAL LAYOUTS
  const renderVerticalDropZones = () => {
    const isThreeRows = dragDropData.dropZones.length === 5;
    const isTwoRows = dragDropData.dropZones.length === 3;

    if (isThreeRows) {
      return React.createElement("div", { className: "dd-vertical-zones" },
        React.createElement("div", { className: "dd-vertical-row" },
        ...
        React.createElement("div", { className: "dd-vertical-row" },
          renderDropZone(dragDropData.dropZones[3]),
          renderDropZone(dragDropData.dropZones[4])
        )
      );
    }
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      showQuestionInLeft && React.createElement("div", { className: "dd-question-row" }, APP_DATA.questionText),
      React.createElement(
        "div",
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement("div", { className: "dd-draggables-row statements" },
          dragDropData.draggables.filter(d => d.text.length > 2).map(renderDraggable)
        ),
        React.createElement("div", { className: "dd-draggables-row operators" },
          dragDropData.draggables.filter(d => d.text.length <= 2).map(renderDraggable)
        )
      )
    ),
