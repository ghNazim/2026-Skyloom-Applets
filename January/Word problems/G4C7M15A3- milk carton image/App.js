const App = () => {
  ...
  useEffect(() => {
    if (currentStep === 1) {
      ...
      if (comprehendSubstep < givenCount) {
        // Given items - show orange highlight only if not null
        const h = comprehendData.given.highlights[comprehendSubstep];
        setCurrentHighlights(h != null ? [h] : null);
      ...
      // Nav text: default navText, when next substep is toFind use navToFind, when all ended use navTextCorrect
  ...
  const handleNext = () => {
    ...
    // Final step (step 8) - play congrats and restart
    if (currentStep === 8) {
      handleRestart();
    } else {
  ...
  // Step 0: Comprehend question only - full width content column + image row + comprehend question
  if (currentStep === 0 && stepData?.isComprehendQuestion) {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "div",
          { className: "full-width-content-column step0-with-image", style: { flexDirection: "column" } },
          React.createElement(
            "div",
          ...
          React.createElement(
            "div",
            { className: "comprehend-question-text", style: { flex: "0 0 30%", display: "flex", alignItems: "center", justifyContent: "center", padding: "1vw" } },
