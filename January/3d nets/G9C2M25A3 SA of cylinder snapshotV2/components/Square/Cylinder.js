...
const T = {
  parameters: {
    ...
    darkBrownDehighlight: 0x3e2345, // dark brown when curved is highlighted (bases full opacity)
  },
};
...
const CylinderVisual = ({
  ...
  useEffect(() => {
    ...
    const updateLabels = (currentUnfoldValue) => {
      ...
      defs.appendChild(
        createMarker("arr-end-c", 3.5, "0 0, 4 2, 0 4", "#00ffff")
      );
      ...
      // ---- Helper: cyclic arrow (elliptical arc with arrow) ----
      const createCyclicArrow = (
        center3D,
        radius,
        startAngle,
        endAngle,
        ...
        for (let i = 0; i <= numPoints; i++) {
          ...
          const x = center3D.x + radius * Math.cos(angle);
          const y = center3D.y + 0.4;
          const z = center3D.z + radius * Math.sin(angle);
          const projected = project3DTo2D(new THREE.Vector3(x, y, z));
        ...
        // Use cyan marker if color is cyan, otherwise white
        const markerId = color === "#00ffff" ? "arr-end-c" : "arr-end-w";
        ...
        if (label) {
          ...
          const labelX = center3D.x + radius * Math.cos(midAngle) * 1.2;
          const labelY = center3D.y + 1;
          const labelZ = center3D.z + radius * Math.sin(midAngle) * 1.2;
          ...
          text.setAttribute("x", labelPos.x + labelOffset.x);
          text.setAttribute("y", labelPos.y + labelOffset.y);
      ...
      // ==============================================================
      // FOLDED STATE LABELS (only shown for highlighted surface during animation)
      // ==============================================================
      if (isFullyFolded && state.highlightPhase) {
        const phase = state.highlightPhase;
        const HIGHLIGHT_CYAN = "#00ffff";
        const highlightedFontSize = "2vw"; // 25% larger (1.6 * 1.25)

        // Show only the labels for the currently highlighted surface
        if (phase === "curved") {
          // Height line (dashed, center bottom to center top)
          createDashedLine(
            new THREE.Vector3(r + 0.2, -h / 2, 0),
            new THREE.Vector3(r + 0.2, h / 2, 0),
            HIGHLIGHT_CYAN,
            1.5
          );
          createLabel(new THREE.Vector3(r + 0.2, 0, 0), "h", {
            offset: { x: 12, y: 0 },
            fontStyle: "italic",
          ...
          // Cyclic arrow around top circle showing circumference (2πr)
          // Positioned above the top flap to avoid overlap
          // Draw arc from about -45 degrees to 225 degrees (270 degrees total, clockwise)
          createCyclicArrow(
            new THREE.Vector3(0, h / 2 + 0.4, r), // Moved up by 0.4 units
            r * 1.0, // Reduced by ~13% from 1.15 (10-15% reduction)
            -Math.PI / 4, // Start angle (-45 degrees)
            (5 * Math.PI) / 4, // End angle (225 degrees)
            "2\u03C0r",
            {
              color: HIGHLIGHT_CYAN,
              strokeWidth: 1.5,
  ...
  useEffect(() => {
    ...
    threeRef.current.highlightPhase = highlightPhase;
  }, [
    unfoldValue,
    showRectBorders,
    showHeightArrow,
  ...
  useEffect(() => {
    ...
    const darkBrown = T.parameters.darkBrownDehighlight;
    ...
    if (highlightPhase === "top") {
      ...
      topBaseMaterial.color.setHex(darkBrown);
      bottomBaseMaterial.color.setHex(darkBrown);
  ...
  useEffect(() => {
    ...
    if (highlightPhase) {
      ...
      // Exception: when curved is highlighted, flaps stay full opacity (with dark brown color)
      ...
      if (highlightPhase === "top") {
        ...
        // Bases stay full opacity with dark brown color (no transparency)
        // top and bottom already opacity 1 from reset above
      }
    } else {
