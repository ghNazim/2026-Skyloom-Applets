...
const CylinderVisual = ({
  ...
  useEffect(() => {
    ...
    const updateLabels = (currentUnfoldValue) => {
      ...
      // ==============================================================
      // FOLDED STATE LABELS (highlight phase: cyan for active part)
      // ==============================================================
      if (isFullyFolded && state.showFoldedLabels) {
        const phase = state.highlightPhase || null;
        const HIGHLIGHT_CYAN = "#00ffff";
        const defaultColor = "#ffffff";
        const defaultFontSize = "1.6vw";
        const highlightedFontSize = "2vw"; // 25% larger (1.6 * 1.25)

        const heightLineColor = phase === "curved" ? HIGHLIGHT_CYAN : defaultColor;
        const heightLabelColor = phase === "curved" ? HIGHLIGHT_CYAN : defaultColor;
        const heightFontSize = phase === "curved" ? highlightedFontSize : defaultFontSize;
        const topRadiusColor = phase === "top" ? HIGHLIGHT_CYAN : defaultColor;
        const topRLabelColor = phase === "top" ? HIGHLIGHT_CYAN : defaultColor;
        const topFontSize = phase === "top" ? highlightedFontSize : defaultFontSize;
        const bottomRadiusColor = phase === "bottom" ? HIGHLIGHT_CYAN : defaultColor;
        const bottomRLabelColor = phase === "bottom" ? HIGHLIGHT_CYAN : defaultColor;
        const bottomFontSize = phase === "bottom" ? highlightedFontSize : defaultFontSize;
  ...
  useEffect(() => {
    ...
    if (highlightPhase) {
      // During highlight: highlighted face full opacity; for top/bottom phases the others at 0.15; for curved phase the two bases stay full opacity (dark brown color set in other effect)
