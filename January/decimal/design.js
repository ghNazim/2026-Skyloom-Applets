...
const App = () => {
  ...
  const triggerTransition = (newPos) => {
    ...
    // 1. Swap Animation Logic (Top Chart)
    ...
    if (dot) {
      ...
      gsap.set(dot, { x: 0, y: 0 }); // Start clean
      const tl = gsap.timeline();
      tl.to(dot, {
        x: totalShift,
        y: "2.5vw", // Arcs underneath
        duration: duration / 2,
        ease: "power1.out",
      }).to(dot, {
        y: 0,
        ...
        ease: "power1.in",
        onComplete: () => {
          // Sync state and reset transforms to avoid flicker
          setPos(newPos);
          setIsMoving(false);
  ...
  return (
    <div ref={containerRef} className="app-wrapper">
      <style>{`
        ...
        .char-slot { width: 4.5vw; display: flex; justify-content: center; align-items: center; position: relative; }
