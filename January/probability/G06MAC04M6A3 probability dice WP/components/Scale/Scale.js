const Scale = ({
  step,
  allVisible,
  showImages,
  customImages,
  highlighted,
  className,
}) => {
  const shouldShowImages = showImages !== false;
  const scalePositions = APP_DATA.scalePositions;
  const images = customImages || APP_DATA.scaleImages;
  const getIndexPct = (idx) => idx * 25;

  const labelsRow = React.createElement(
    "div",
    { className: "scale-labels-row" },
    scalePositions.map((pos, i) =>
      React.createElement(
        "div",
        {
          key: i,
          className: "scale-label-wrapper",
          style: {
            left: getIndexPct(i) + "%",
            opacity: allVisible ? 1 : step === i + 1 ? 1 : 0,
            pointerEvents: allVisible || step === i + 1 ? "auto" : "none",
          },
        },
        React.createElement("div", { className: "scale-label-bubble" + (current_language === "id" ? " id" : "") }, pos.label)
      )
    )
  );

  const trackRow = React.createElement(
    "div",
    { className: "scale-track-row" },
    React.createElement("div", { className: "scale-track-line" }),
    scalePositions.map((pos, i) =>
      React.createElement("div", {
        key: i,
        className: "scale-dot",
        style: {
          left: getIndexPct(i) + "%",
          backgroundColor: pos.dotColor,
          boxShadow: "0 0 0.4vw " + pos.dotColor,
        },
      })
    )
  );

  const imagesRow = shouldShowImages
    ? React.createElement(
        "div",
        { className: "scale-images-row" },
        images.map(function (img, i) {
          if (!img) return null;

          var isGroup = Array.isArray(img);

          return React.createElement(
            "div",
            {
              key: i,
              className: "scale-image-wrapper",
              style: {
                left: getIndexPct(i) + "%",
                opacity: allVisible ? 1 : step === i + 1 ? 1 : 0.4,
              },
            },
            isGroup
              ? React.createElement(
                  "div",
                  { style: { display: "flex", gap: "0" } },
                  img.map(function (subImg, j) {
                    return React.createElement("img", {
                      key: j,
                      src: subImg,
                      className: "scale-image in-group",
                      draggable: false,
                      alt: "",
                    });
                  })
                )
              : React.createElement("img", {
                  src: img,
                  className: "scale-image",
                  draggable: false,
                  alt: scalePositions[i] ? scalePositions[i].label : "",
                })
          );
        })
      )
    : null;

  return React.createElement(
    "div",
    {
      className:
        "scale-component" +
        (shouldShowImages ? "" : " scale-compact") +
        (highlighted ? " scale-highlighted" : "") +
        (className ? " " + className : ""),
    },
    labelsRow,
    trackRow,
    imagesRow
  );
};
